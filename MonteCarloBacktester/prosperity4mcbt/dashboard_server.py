...
def _find_frontend_dist() -> Path | None:
    """Find the built visualizer dist/ directory."""
    # Check relative to this file: ../../visualizer/dist
    here = Path(__file__).resolve().parent
    candidates = [
        here.parent.parent / "visualizer" / "dist",
        here.parent / "visualizer" / "dist",
    ]
    # Also check PROSPERITY4MCBT_ROOT env var
    root_env = os.environ.get("PROSPERITY4MCBT_ROOT")
    if root_env:
        candidates.insert(0, Path(root_env) / "visualizer" / "dist")
    for c in candidates:
...
class DashboardRequestHandler(SimpleHTTPRequestHandler):
    _frontend_dist: Path | None = _find_frontend_dist()
    ...
    def do_GET(self) -> None:
        ...
        # Try serving from data directory first (session files, etc.)
        data_dir = Path(getattr(self, "directory", ".")).resolve()
        data_file = data_dir / path.lstrip("/")
        if data_file.is_file():
            super().do_GET()
            return

        # Serve from frontend dist/ if available
        if self._frontend_dist is not None:
            frontend_file = self._frontend_dist / path.lstrip("/")
            if frontend_file.is_file():
                self._serve_file(frontend_file)
                return

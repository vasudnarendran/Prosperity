...
def _is_repo_root(path: Path) -> bool:
    return (path / "backtester" / "prosperity3bt").is_dir() and (path / "rust_simulator").is_dir()


def _is_release_root(path: Path) -> bool:
    return (path / "prosperity3bt").is_dir() and (path / "prosperity4mcbt").is_dir() and (path / "bin").is_dir()
...
def bundled_simulator_path() -> Optional[Path]:
    root = project_root()
    if not _is_release_root(root):
        return None

    binary_name = "prosperity4_sim.exe" if os.name == "nt" else "prosperity4_sim"
...
def downsample_path_node(node: dict[str, list[float] | list[int]], max_points: int) -> dict[str, list[float] | list[int]]:
    if not node.get("timestamps"):
        return node
    indices = downsample_indices(len(node["timestamps"]), max_points)
    return {key: [values[index] for index in indices] if values else [] for key, values in node.items()}
...
def run_rust_monte_carlo(
    ...
    if day:
        args.extend(["--day", day])
    if leaderboard_submit:
        args.append("--leaderboard-submit")
    if display_name:
        args.extend(["--display-name", display_name])

    simulator_binary = bundled_simulator_path()
    if simulator_binary is not None:
        cmd = [str(simulator_binary.resolve()), *args]
        cwd = project_root()
    else:
        simulator_dir = rust_dir()
        if not simulator_dir.is_dir():
            raise RuntimeError(
                f"Neither a bundled simulator binary nor a rust_simulator source tree was found under {project_root()}. "
                "Release bundles should include bin/prosperity4_sim, and development checkouts should include rust_simulator/."
            )
        cmd = [
            "cargo",
            "run",

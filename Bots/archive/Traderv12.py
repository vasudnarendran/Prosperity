...
class EmeraldsTrader(BaseProductTrader):
    ...
    def passive_quotes(self, adjusted_fair: float) -> Tuple[Optional[int], Optional[int]]:
        ...
        if int(self.best_bid) >= 10000 and self.position >= 0:
            sell_quote += 1
    ...
    def run(self) -> List[Order]:
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY") and self.buy_capacity > 0:
            ...
            if phase == "early":
                ...
                take_limit = max(4, take_limit - 3)
            if phase != "late" or self.position < late_position_cap:
...
class TomatoesTrader(BaseProductTrader):
    ...
    def target_band(self, regime: str) -> Tuple[int, int]:
        ...
        if regime == "trend_up":
            band = (20, 36) if self.momentum >= 4.0 else (10, 26)
        elif regime == "trend_down":
            band = (-36, -20) if self.momentum <= -4.0 else (-26, -10)
        elif regime == "toxic":
            band = (-8, 8)
        else:
            band = (-10, 10)
        ...
        if phase == "late":
            lower, upper = band
            return max(lower, -18), min(upper, 18)
        return band

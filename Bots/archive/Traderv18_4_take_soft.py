...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "EXIT_TAKE_HOLD_EDGE": 1.75,
    "EXIT_TAKE_PENALTY": 0.25,
    "EXIT_TAKE_RELEASE_EDGE": -1.25,
    "EXIT_TAKE_RELEASE_BONUS": 0.10,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def take_edge(
        ...
        if side == "SELL":
            if (
                predicted_edge >= self.EXIT_TAKE_HOLD_EDGE
                and fit_quality >= self.FIT_THRESHOLD
                and regime != "trend_down"
            ):
                edge += self.EXIT_TAKE_PENALTY
            elif predicted_edge <= self.EXIT_TAKE_RELEASE_EDGE:
                edge -= self.EXIT_TAKE_RELEASE_BONUS

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "LATE_START_TS": 165000.0,
    "LATE_PASSIVE_WRONGWAY_BLOCK": 1.0,
    "LATE_EXIT_EDGE_RELIEF": 0.20,
    "LATE_HOLD_BONUS_DAMP": 0.45,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def is_late_session(self) -> bool:
        return float(getattr(self.state, "timestamp", 0)) >= self.LATE_START_TS
    ...
    def passive_quotes(
        ...
        if mode == "strong_up":
            ...
            if position > 0:
                ...
                sell_quote = None
        if self.is_late_session() and self.LATE_PASSIVE_WRONGWAY_BLOCK > 0:
            if position > 0 and predicted_edge <= 0:
                buy_quote = None
            elif position < 0 and predicted_edge >= 0:
                sell_quote = None
        if regime == "trend_up":
    ...
    def take_orders(
        ...
        if (
            ...
            if mode in {"lean_down", "strong_down", "defensive"}:
                ...
                if self.is_late_session() and self.projected_position() < 0 and predicted_edge >= -self.TREND_EDGE_THRESHOLD:
                    quantity = min(quantity, max(1, abs(self.projected_position())))
                before = self.buy_capacity
                self.add_buy(int(self.best_ask), quantity)
                took_buy = self.buy_capacity < before
        ...
        if (
            ...
            if self.is_late_session():
                required_bonus *= self.LATE_HOLD_BONUS_DAMP
                if self.projected_position() > 0 and predicted_edge <= self.TREND_EDGE_THRESHOLD:
                    required_bonus = max(0.0, required_bonus - self.LATE_EXIT_EDGE_RELIEF)

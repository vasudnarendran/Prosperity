...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_RESERVATION_BIAS": 0.18,
    "RANGE_RESERVATION_BIAS": 0.12,
    "ALPHA_EDGE_SCALE": 1.06,
    "ALPHA_IMBALANCE_SCALE": 1.08,
    "ALPHA_THRESHOLD_SCALE": 0.95,
    "SOFT_LIMIT_BASE_RATIO": 0.65,
    "SOFT_LIMIT_TREND_BONUS": 0.00,
    "SOFT_LIMIT_VOL_PENALTY": 0.00,
    "SOFT_LIMIT_FIT_BONUS": 0.00,
    "TAKE_VOL_COEF": 0.00,
    "TAKE_INV_COEF": 0.00,
    "TAKE_TIME_COEF": 0.00,
    "HOLD_TIME_COEF": 0.00,
    "HOLD_VOL_COEF": 0.00,
    "TREND_SELL_HOLD_EXTRA": 0.00,
    "TREND_BUY_TAKE_EXTRA": 0.00,
    "TREND_QUOTE_LIFT_EXTRA": 0.00,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def effective_soft_limit(self, regime: str, fit_quality: float, volatility: float) -> int:
        ratio = self.SOFT_LIMIT_BASE_RATIO
        if regime in {"trend_up", "trend_down"}:
            ratio += self.SOFT_LIMIT_TREND_BONUS
        if regime == "volatile":
            ratio -= self.SOFT_LIMIT_VOL_PENALTY
        ratio += self.SOFT_LIMIT_FIT_BONUS * fit_quality
        ratio = max(0.35, min(0.95, ratio))
        return min(self.position_limit, max(8, int(self.position_limit * ratio)))
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        dynamic_limit = self.effective_soft_limit(regime, fit_quality, self.realized_volatility(int(self.VOL_CONTROL_WINDOW)))
        lower = max(lower, -dynamic_limit)
        upper = min(upper, dynamic_limit)
        position = self.projected_position()
        if position < lower:
            return lower
        if position > upper:
    ...
    def take_edge(
        ...
        edge += self.TAKE_VOL_COEF * min(3.0, volatility)
        edge += self.TAKE_INV_COEF * min(1.0, target_gap / max(1, self.position_limit))
        edge += self.TAKE_TIME_COEF * (1.0 - tau)
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
        soft_limit = self.effective_soft_limit(regime, fit_quality, volatility)
        edge = max(self.BASE_QUOTE_EDGE, float(self.spread) / 3.5)
        edge = min(self.MAX_QUOTE_EDGE, edge)

        if abs(self.projected_position()) >= soft_limit:
            edge += 0.5
    ...
    def allow_passive(self, side: str, regime: str) -> bool:
        dynamic_limit = self.effective_soft_limit(regime, 0.6, self.realized_volatility(int(self.VOL_CONTROL_WINDOW)))
        position = self.projected_position()
        if side == "BUY" and position >= dynamic_limit:
            return False
        if side == "SELL" and position <= -dynamic_limit:
            return False
...
class Trader:
    ...
    def load_param_overrides(self) -> Dict[str, Dict[str, float]]:
        ...
        for product, values in parsed.items():
            ...
            for key, value in values.items():
                if isinstance(value, (int, float)):
                    product_values[key] = float(value)
            if product_values:
                cleaned[product] = product_values
        return cleaned
    ...
    def run(self, state: TradingState):
        ...
        param_overrides = self.load_param_overrides()

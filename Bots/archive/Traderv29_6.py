...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "AS_OVERLAY_GAMMA_RANGE": 0.04,
    "AS_OVERLAY_GAMMA_TREND": 0.025,
    "AS_OVERLAY_GAMMA_VOLATILE": 0.06,
    "AS_OVERLAY_SCALE": 0.22,
    "AS_OVERLAY_MAX_SHIFT": 1.25,
    "GAMMA_RANGE": 0.34,
    "GAMMA_TREND": 0.10,
    "GAMMA_VOLATILE": 0.40,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def overlay_gamma(self, regime: str) -> float:
        if regime == "trend_up" or regime == "trend_down":
            return self.AS_OVERLAY_GAMMA_TREND
        if regime == "volatile":
            return self.AS_OVERLAY_GAMMA_VOLATILE
        return self.AS_OVERLAY_GAMMA_RANGE

    def as_risk_adjustment(
        self,
        regime: str,
        target_position: int,
        volatility: float,
        ...
        gamma = self.overlay_gamma(regime)
        tau = self.time_fraction_remaining()
        sigma = max(0.8, volatility)
        reservation_shift = (
            inventory_gap
            * gamma
            * sigma
            * self.AS_OVERLAY_SCALE
            * max(0.35, tau)
        )
        return max(-self.AS_OVERLAY_MAX_SHIFT, min(self.AS_OVERLAY_MAX_SHIFT, reservation_shift))
    ...
    def run(self) -> List[Order]:
        ...
        signal_fair = self.adjusted_fair_value(
            regime,
            target_position,
            predicted_now,
            predicted_next,
        )
        quote_fair = signal_fair - self.as_risk_adjustment(regime, target_position, volatility)

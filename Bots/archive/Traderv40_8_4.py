...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "REENTRY_IMBALANCE_THRESHOLD": 0.10,
    "REENTRY_EDGE_BONUS": 0.16,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def trend_reentry_allowed(
        ...
        position = self.projected_position()
        light_cap = max(10, int(0.40 * self.soft_limit))
        if side == "BUY":
            return (
                regime == "trend_up"
        ...
        if (
            ...
            if mode in {"lean_down", "strong_down", "defensive"}:
                ...
                if regime != "range":
                    ...
                    if reentry_buy:
                        desired_target = max(desired_target, max(10, int(0.45 * self.soft_limit)))
        ...
        if (
            ...
            if mode in {"lean_up", "strong_up", "defensive"} and self.projected_position() <= max(target_position, 0):
                ...
                if regime != "range":
                    desired_target = target_position
                    if reentry_sell:
                        desired_target = min(desired_target, -max(10, int(0.45 * self.soft_limit)))

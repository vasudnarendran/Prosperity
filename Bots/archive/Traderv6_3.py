...
class Trader:
    ...
    # Slightly more selective on EMERALDS taking.
    ...
    # Lower divisor means wider quotes, higher divisor means tighter quotes.
    QUOTE_SPREAD_DIVISOR: Dict[str, float] = {
        "EMERALDS": 4.0,
        "TOMATOES": 4.0,
    }
    ...
    def load_trader_data(self, trader_data: str) -> Dict[str, List[float]]:
        ...
        for product, values in history.items():
            if not isinstance(values, list):
                continue
            cleaned_history[product] = [float(value) for value in values[-4:]]
    ...
    def get_microprice(
        ...
        return (
            (best_bid * best_ask_amount) + (best_ask * best_bid_amount)
        ) / total_size
    ...
    def get_fair_value(
        ...
        if best_bid is not None and best_ask is not None:
            ...
            if reference_price is None:
                ...
                recent_average = (
                    sum(recent_mids) / len(recent_mids)
                    if recent_mids
                    else mid_price
                )
                return 0.55 * mid_price + 0.30 * microprice + 0.15 * recent_average
    ...
    def get_quote_edge(self, product: str, spread: int) -> float:
        ...
        spread_divisor = self.QUOTE_SPREAD_DIVISOR.get(product, 4.0)
        return min(max_edge, max(base_edge, spread / spread_divisor))

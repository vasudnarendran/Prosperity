...
class TomatoesTrader(BaseProductTrader):
    ...
    def guarded_hybrid_alpha(self, hybrid_alpha: float, regression_edge: float, regime: str) -> float:
        ...
        if regime == "range":
            weight *= min(1.0, self.RANGE_ALPHA_DAMP + (self.BUY_RANGE_RELIEF if buy_relief else 0.0))

        if hybrid_alpha * regression_edge < 0:
            weight *= min(1.0, self.CONFLICT_ALPHA_DAMP + (self.BUY_CONFLICT_RELIEF if buy_relief else 0.0))
        ...
        if hybrid_alpha * self.momentum < 0:
            ...
            weight *= min(1.0, self.MOMENTUM_ALPHA_DAMP + momentum_relief)

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "INVENTORY_RESERVATION_SKEW": 0.010,
    "SIZE_INVENTORY_PRESSURE": 0.40,
    "BASE_TAKE_EDGE": 0.82,
    "BASE_QUOTE_EDGE": 2.72,
    "PASSIVE_SIZE": 8,
    "MAX_TAKE_SIZE": 10,
    "REGRESSION_WINDOW": 8,
    "REGRESSION_HORIZON": 0.5,
    "TREND_ENTRY_THRESHOLD": 0.95,
    "TREND_STRONG_THRESHOLD": 1.85,
    "FIT_THRESHOLD": 0.45,
    "TOXIC_SPREAD_THRESHOLD": 15.0,
    "TOXIC_VOL_THRESHOLD": 3.2,
    "REGIME_SMOOTH_ALPHA": 0.26,
    "SOFT_LIMIT_RATIO": 0.56828726,
    "TARGET_TREND_RATIO": 0.58,
    "TARGET_RANGE_RATIO": 0.18,
    "TARGET_STRETCH_DAMP": 0.20,
    "TIME_HORIZON_TICKS": 10000.0,
    "LATE_FLATTEN_START": 0.18,
    "LATE_FLATTEN_HARD": 0.08,
    "QUOTE_VOL_COEF": 0.12,
    "QUOTE_TOXIC_COEF": 1.10,
    "QUOTE_FEEDBACK_COEF": 0.18,
    "QUOTE_TREND_SKEW": 0.35,
    "TAKE_TREND_REWARD": 0.28,
    "TAKE_TOXIC_PENALTY": 0.22,
    "TAKE_FEEDBACK_PENALTY": 0.20,
    ...
    "GOOD_FILL_BONUS": 0.05,
    "BAD_FILL_PENALTY": 0.20,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def state_overlay(self, state_name: str, volatility: float, fit_quality: float) -> Dict[str, float]:
        calm = (
            state_name == "range"
            and float(self.spread) <= 12
            and volatility <= 1.9
            and abs(self.imbalance) <= 0.20
            and self.inventory_pressure() <= 0.30
        )
        fast = state_name in {"trend_up", "trend_down", "strong_up", "strong_down"}
        overlay = {
            "join_ticks": 0.0,
            "quote_mult": 1.0,
            "take_shift": 0.0,
        ...
        if calm and self.adverse_ewma < 0.45:
            overlay["join_ticks"] = 1.0
            overlay["quote_mult"] *= 0.97

        if fast and fit_quality >= self.FIT_THRESHOLD and self.toxic_score < 0.75:
            conviction = clip(abs(self.trend_score) / max(self.TREND_ENTRY_THRESHOLD, 1.0), 0.0, 1.0)
            overlay["take_shift"] -= 0.08 * conviction
            if self.inventory_pressure() <= 0.60:
                overlay["target_bonus"] += 2.0 * conviction
            overlay["size_mult"] *= 1.0 + 0.10 * conviction

        if self.fill_quality_ewma > 0.20 and self.adverse_ewma < 0.30 and self.toxic_score < 0.50:
            overlay["take_shift"] -= 0.04
            overlay["quote_mult"] *= 0.98
            overlay["size_mult"] *= 1.05

        if self.adverse_ewma > 0.35:
            adverse_scale = clip((self.adverse_ewma - 0.35) / 0.45, 0.0, 1.0)
            overlay["take_shift"] += 0.12 * adverse_scale
            overlay["quote_mult"] *= 1.0 + 0.08 * adverse_scale
            overlay["size_mult"] *= 1.0 - 0.12 * adverse_scale
    ...
    def classify_state(self, fit_quality: float) -> str:
        ...
        if self.toxic_score >= 0.95:
            return "volatile"
        if (
            self.trend_score >= self.TREND_STRONG_THRESHOLD
            and fit_quality >= self.FIT_THRESHOLD
            and self.imbalance > 0.05
    ...
    def target_position(self, state_name: str, predicted_edge: float, overlay: Dict[str, float]) -> int:
        ...
        if state_name == "volatile":
            return 0
        if state_name in {"strong_up", "strong_down", "trend_up", "trend_down"}:
            conviction = clip(abs(self.trend_score) / max(self.TREND_ENTRY_THRESHOLD, self.TREND_STRONG_THRESHOLD), 0.35, 1.0)
            target = int(round(self.soft_limit * self.TARGET_TREND_RATIO * conviction + overlay["target_bonus"]))
            target = int(round(target * max(0.55, 1.0 - self.TARGET_STRETCH_DAMP * max(0.0, abs(stretch) - 1.0))))
            if tau <= self.LATE_FLATTEN_START:
                target = int(round(target * max(0.35, tau / self.LATE_FLATTEN_START)))
            return target if state_name in {"strong_up", "trend_up"} else -target
    ...
    def take_edge(
        ...
        if side == "BUY" and state_name in {"trend_up", "strong_up"}:
            edge -= self.TAKE_TREND_REWARD
        elif side == "SELL" and state_name in {"trend_down", "strong_down"}:
            edge -= self.TAKE_TREND_REWARD
        elif state_name in {"strong_up", "strong_down"}:
            edge += 0.22
        ...
        if predicted_edge > 0 and side == "BUY":
            edge -= min(0.16, 0.05 * predicted_edge)
        elif predicted_edge < 0 and side == "SELL":
            edge -= min(0.16, 0.05 * abs(predicted_edge))
        edge += overlay["take_shift"]
        return max(0.45, edge)

    def quote_edge(self, state_name: str, volatility: float, overlay: Dict[str, float]) -> float:
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(state_name, volatility, overlay)
        center = reservation_price
        if state_name in {"trend_up", "strong_up"}:
            center += self.QUOTE_TREND_SKEW * clip(predicted_edge / max(1.0, self.TREND_ENTRY_THRESHOLD), 0.0, 1.0)
        elif state_name in {"trend_down", "strong_down"}:
        ...
        if overlay["join_ticks"] > 0:
            if buy_quote is not None:
                buy_quote = min(int(self.best_ask) - 1, buy_quote + int(overlay["join_ticks"]))
            if sell_quote is not None:
                sell_quote = max(int(self.best_bid) + 1, sell_quote - int(overlay["join_ticks"]))

        if state_name == "strong_up":
            sell_quote = None
        elif state_name == "strong_down":
            buy_quote = None
        elif state_name == "trend_up" and self.projected_position() <= target_position:
            sell_quote = None
    ...
    def passive_size(self, side: str, state_name: str, overlay: Dict[str, float]) -> int:
        ...
        size = int(round(size * overlay["size_mult"]))
    ...
    def sweep_side(
        ...
        threshold = self.take_edge(side, state_name, predicted_edge, volatility, overlay)
        for price, volume in levels:
            ...
            if side == "BUY":
                ...
                if state_name in {"strong_up", "trend_up"} and desired <= 0:
                    continue
                take_limit = int(self.MAX_TAKE_SIZE)
                if state_name == "strong_down":
                    take_limit += 2
    ...
    def run(self) -> List[Order]:
        ...
        overlay = self.state_overlay(state_name, volatility, fit_quality)
        target_position = self.target_position(state_name, predicted_edge, overlay)
        ...
        took_buy = self.sweep_side("BUY", state_name, target_position, reservation_price, predicted_edge, volatility, overlay)
        took_sell = self.sweep_side("SELL", state_name, target_position, reservation_price, predicted_edge, volatility, overlay)
        ...
        if (
            ...
            if state_name == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", state_name, overlay), self.buy_capacity)
                if state_name != "range":
        ...
        if (
            ...
            if state_name == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", state_name, overlay), self.sell_capacity)
                if state_name != "range":

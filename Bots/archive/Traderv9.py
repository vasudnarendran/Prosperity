...
class Trader:
    ...
    def get_market_state(
        ...
        short_average = recent_mids[-3:] if recent_mids else []
        short_anchor = sum(short_average) / len(short_average) if short_average else mid

        return {
            "mid": mid,
            "micro": micro,
            "recent_average": recent_average,
            "short_average": short_anchor,
            "momentum": mid - recent_average,
            "short_momentum": mid - short_anchor,
            "imbalance": imbalance,
            "spread": float(spread),
    ...
    def classify_tomatoes_state(self, market_state: Dict[str, float]) -> str:
        momentum = market_state["momentum"]
        short_momentum = market_state["short_momentum"]
        imbalance = market_state["imbalance"]
        spread = market_state["spread"]
        micro = market_state["micro"]
        mid = market_state["mid"]

        if spread >= 15 and abs(momentum) < 1.5:
            return "toxic"
        if momentum >= 1.5 and imbalance >= 0.15 and micro >= mid:
            return "trend_up"
        if momentum <= -1.5 and imbalance <= -0.15 and micro <= mid:
            return "trend_down"
        return "mean_revert"

    def get_tomatoes_target_band(
        self,
        state_name: str,
        market_state: Dict[str, float],
    ) -> Tuple[int, int]:
        if state_name == "trend_up":
            if market_state["momentum"] >= 4.0:
                return 20, 36
            return 10, 26
    ...
    def get_tomatoes_band_target(
        self,
        state_name: str,
        market_state: Dict[str, float],
        position: int,
    ) -> int:
        lower, upper = self.get_tomatoes_target_band(state_name, market_state)
        if position < lower:
            return lower
        if position > upper:
            return upper
    ...
    def get_take_edge(
        ...
        position: int,
        market_state: Dict[str, float],
        tomatoes_state: Optional[str] = None,
    ) -> float:
        ...
        if product == "TOMATOES":
            if tomatoes_state == "trend_up":
                if side == "BUY":
                    edge -= 0.4
                else:
                    edge += 0.6
            elif tomatoes_state == "trend_down":
                if side == "SELL":
                    edge -= 0.4
                else:
                    edge += 0.6
            elif tomatoes_state == "toxic":
                edge += 0.5
            else:
                if side == "BUY" and market_state["momentum"] <= -2.0:
                    edge += 0.5
                elif side == "SELL" and market_state["momentum"] >= 2.0:
                    edge += 0.5
            edge += 0.20 * self.get_toxicity(product, market_state)
        else:
    ...
    def get_quote_edge(
        ...
        if product == "TOMATOES":
            if tomatoes_state == "toxic":
                quote_edge += 1.0
            elif tomatoes_state in {"trend_up", "trend_down"}:
                quote_edge += 0.25
            quote_edge += 0.35 * self.get_toxicity(product, market_state)
    ...
    def get_passive_quotes(
        ...
        market_state: Dict[str, float],
        tomatoes_state: Optional[str] = None,
        tomatoes_target: Optional[int] = None,
    ) -> Tuple[Optional[int], Optional[int]]:
        best_bid = int(market_state["best_bid"])
        best_ask = int(market_state["best_ask"])
        quote_edge = self.get_quote_edge(product, position, market_state, tomatoes_state)
        ...
        if product == "EMERALDS":
            ...
            if tomatoes_state == "trend_up":
                if tomatoes_target is not None and position < tomatoes_target:
                    buy_quote = min(best_ask - 1, max(best_bid + 1, buy_quote + 1))
                if position <= 16:
                    sell_quote = None
            elif tomatoes_state == "trend_down":
    ...
    def should_place_passive_order(
        ...
        if product == "TOMATOES" and tomatoes_state == "toxic" and abs(position) <= 6:
            return False
    ...
    def get_passive_size(
        ...
        base_size = self.PARAMS[product]["passive_size"]
        ...
        if product == "TOMATOES":
            if tomatoes_state == "toxic":
                base_size = max(1, base_size - 3)
            elif tomatoes_state in {"trend_up", "trend_down"}:
                base_size = max(1, base_size - 1)
            base_size -= int(self.get_toxicity(product, {"momentum": 0.0, "imbalance": 0.0}))
        elif (
            (side == "BUY" and spread >= 8)
            or (side == "SELL" and spread >= 8)
        ):
            base_size += 1
    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "TOMATOES":
                tomatoes_state = self.classify_tomatoes_state(market_state)
                tomatoes_target = self.get_tomatoes_band_target(tomatoes_state, market_state, position)
                fair_value += (tomatoes_target - position) / 18.0
            ...
            if best_ask <= adjusted_fair - buy_take_edge and buy_capacity > 0:
                ...
                if product == "EMERALDS" and best_ask <= 10000:
                    buy_take_limit += 2
                elif product == "TOMATOES" and tomatoes_state == "trend_up":
                    buy_take_limit += 2
                buy_volume = min(best_ask_amount, buy_take_limit, buy_capacity)
                if buy_volume > 0:
                    if product != "TOMATOES" or tomatoes_target is None or position < tomatoes_target:
            ...
            if best_bid >= adjusted_fair + sell_take_edge and sell_capacity > 0:
                ...
                if product == "EMERALDS" and best_bid >= 10000:
                    sell_take_limit += 2
                elif product == "TOMATOES" and tomatoes_state == "trend_down":
                    sell_take_limit += 2
            ...
            allow_mean_revert_quotes = product == "TOMATOES" and tomatoes_state == "mean_revert"

            if (
                not took_buy
                and buy_quote is not None
                and buy_capacity > 0
                and self.should_place_passive_order("BUY", position, limit, product, tomatoes_state)
            ):
                if (
                    allow_mean_revert_quotes
                    or product != "TOMATOES"
                    or tomatoes_target is None
                    or position < tomatoes_target
                ):
                    passive_buy_size = min(
                        self.get_passive_size(product, position, "BUY", best_ask - best_bid, tomatoes_state),
                        buy_capacity,
                    )
                    if tomatoes_target is not None and not allow_mean_revert_quotes:
                        passive_buy_size = min(passive_buy_size, max(1, tomatoes_target - position))
            ...
            if (
                ...
                and sell_capacity > 0
                and self.should_place_passive_order("SELL", position, limit, product, tomatoes_state)
            ):
                if (
                    allow_mean_revert_quotes
                    or product != "TOMATOES"
                    ...
                    passive_sell_size = min(
                        self.get_passive_size(product, position, "SELL", best_ask - best_bid, tomatoes_state),
                        sell_capacity,
                    )
                    if tomatoes_target is not None and not allow_mean_revert_quotes:
                        passive_sell_size = min(passive_sell_size, max(1, position - tomatoes_target))

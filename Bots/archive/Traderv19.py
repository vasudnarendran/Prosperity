...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MAKER_SPREAD_DIVISOR": 4.50,
    "MAKER_JOIN_THRESHOLD": 1.00,
    "TREND_HOLD_LIFT": 1.0,
    "CLEAN_SPREAD_PASSIVE_BONUS": 1.0,
    "EXIT_HOLD_BONUS": 0.35,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def target_band(self, regime: str, predicted_edge: float, fit_quality: float) -> Tuple[int, int]:
        ...
        if regime == "trend_up":
            return (10, 24) if conviction >= self.STRONG_TREND_EDGE else (4, 16)
        if regime == "trend_down":
            return (-24, -10) if conviction >= self.STRONG_TREND_EDGE else (-16, -4)
        if regime == "volatile":
            return -6, 6
        return -12, 12
    ...
    def fair_value(
        ...
        if regime == "range":
            ...
            fair += (0.10 * line_gap) + (0.15 * (predicted_next - float(self.mid)))
        return fair
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float) -> float:
        edge = max(self.BASE_QUOTE_EDGE, float(self.spread) / self.MAKER_SPREAD_DIVISOR)
        ...
        edge += 0.15 * min(2.0, volatility)
        return min(self.MAX_QUOTE_EDGE, max(1.5, edge))
    ...
    def passive_quotes(
        ...
        position = self.projected_position()
        inside_buy = int(self.best_bid) + 1 if int(self.best_bid) + 1 < int(self.best_ask) else None
        inside_sell = int(self.best_ask) - 1 if int(self.best_ask) - 1 > int(self.best_bid) else None

        if inside_buy is not None and buy_quote is not None:
            if inside_buy >= buy_quote - self.MAKER_JOIN_THRESHOLD:
                buy_quote = max(buy_quote, inside_buy)
        if inside_sell is not None and sell_quote is not None:
            if inside_sell <= sell_quote + self.MAKER_JOIN_THRESHOLD:
                sell_quote = min(sell_quote, inside_sell)
        ...
        if regime == "trend_up":
            ...
            if sell_quote is not None and predicted_edge >= self.TREND_EDGE_THRESHOLD and position <= max(8, target_position // 2):
                sell_quote += int(self.TREND_HOLD_LIFT)
        elif regime == "trend_down":
            ...
            if buy_quote is not None and predicted_edge <= -self.TREND_EDGE_THRESHOLD and position >= min(-8, target_position // 2):
                buy_quote -= 1
    ...
    def passive_size(self, side: str, regime: str, volatility: float) -> int:
        ...
        if int(self.spread) >= 14:
            size += 2
        if int(self.spread) >= 12 and self.toxicity(volatility) == 0.0:
            size += int(self.CLEAN_SPREAD_PASSIVE_BONUS)
    ...
    def take_orders(
        ...
        took_buy = False
        took_sell = False
        buy_dislocation = adjusted_fair - int(self.best_ask)
        sell_dislocation = int(self.best_bid) - adjusted_fair

        if (
            buy_dislocation >= self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility)
            and self.buy_capacity > 0
        ):
            take_limit = self.MAX_TAKE_SIZE + (1 if regime == "trend_up" else 0)
            if regime == "trend_down" and predicted_edge <= -self.TREND_EDGE_THRESHOLD:
                take_limit = max(1, take_limit - 1)
        ...
        if (
            sell_dislocation >= self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility)
            and self.sell_capacity > 0
        ):
            take_limit = self.MAX_TAKE_SIZE + (1 if regime == "trend_down" else 0)
            required_dislocation = 0.0
            if regime == "trend_up" and predicted_edge >= self.TREND_EDGE_THRESHOLD:
                required_dislocation += self.EXIT_HOLD_BONUS
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif sell_dislocation < self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility) + required_dislocation:
                pass
            else:

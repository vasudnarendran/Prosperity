...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_BUY_QUOTE_EXTRA": 1.0,
    "TREND_SELL_QUOTE_EXTRA": 1.0,
    "HOLD_TIME_COEF": 0.08,
    "HOLD_VOL_COEF": 0.0,
}
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            if buy_quote is not None and position < target_position:
                push = int(self.TREND_PASSIVE_PUSH)
                if predicted_edge >= self.TREND_EDGE_THRESHOLD and fit_quality >= self.FIT_THRESHOLD:
                    push += int(self.TREND_BUY_QUOTE_EXTRA)
                buy_quote = min(
                    int(self.best_ask) - 1,
                    max(int(self.best_bid) + 1, buy_quote + push),
                )
            if sell_quote is not None and position > 0:
                ...
                if fit_quality >= self.FIT_THRESHOLD:
                    lift += int(self.TREND_SELL_QUOTE_EXTRA)
                sell_quote += lift
        elif regime == "trend_down":
            if sell_quote is not None and position > target_position:
                ...
                sell_quote = max(
                    int(self.best_bid) + 1,
                    min(int(self.best_ask) - 1, sell_quote - push),
                )
            if buy_quote is not None and position < 0:
                ...
                if predicted_edge <= -self.STRONG_TREND_EDGE:
                    drop += 1
                if fit_quality >= self.FIT_THRESHOLD:
                    drop += int(self.TREND_SELL_QUOTE_EXTRA)
                buy_quote -= drop

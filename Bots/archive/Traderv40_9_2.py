...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "RESIDUAL_OVERSTRETCH_THRESHOLD": 1.20,
    "RESIDUAL_BRAKE_STRENGTH": 0.12,
}
    ...
    def take_edge(
        ...
        if regime in {"trend_up", "trend_down"}:
            if side == "BUY" and predicted_edge > 0 and residual > self.RESIDUAL_OVERSTRETCH_THRESHOLD:
                edge += self.RESIDUAL_BRAKE_STRENGTH * min(1.5, residual - self.RESIDUAL_OVERSTRETCH_THRESHOLD)
            elif side == "SELL" and predicted_edge < 0 and residual < -self.RESIDUAL_OVERSTRETCH_THRESHOLD:
                edge += self.RESIDUAL_BRAKE_STRENGTH * min(1.5, abs(residual) - self.RESIDUAL_OVERSTRETCH_THRESHOLD)
    ...
    def take_orders(
        ...
        if (
            ...
            if mode in {"lean_down", "strong_down", "defensive"}:
                ...
                regime == "trend_up"
                and predicted_edge > 0
                and residual > self.RESIDUAL_OVERSTRETCH_THRESHOLD
                and self.projected_position() >= max(6, target_position - 8)
            ):
                pass
            else:
        ...
        if (
            ...
            if mode in {"lean_up", "strong_up", "defensive"} and self.projected_position() <= max(target_position, 0):
                ...
                and predicted_edge < 0
                and residual < -self.RESIDUAL_OVERSTRETCH_THRESHOLD
                and self.projected_position() <= min(-6, target_position + 8)
            ):
                pass

...
class TomatoesTrader(BaseProductTrader):
    ...
    def classify_state(
        ...
        if float(self.spread) >= self.TOXIC_SPREAD_THRESHOLD and volatility >= self.TOXIC_VOLATILITY_THRESHOLD:
            return "volatile"
        if (
            predicted_edge >= self.BUY_TREND_EDGE_THRESHOLD
            ...
            return "trend_up"
        if (
            predicted_edge <= -self.SELL_TREND_EDGE_THRESHOLD
            and fit_quality >= self.FIT_THRESHOLD
            and self.imbalance <= -self.TREND_IMBALANCE_THRESHOLD
            and self.momentum <= -0.75
            and float(self.micro) <= float(self.mid)
    ...
    def take_edge(
        ...
        if regime == "trend_up":
            ...
            if side == "BUY" and predicted_edge <= -self.BUY_TREND_EDGE_THRESHOLD:
                edge += 0.35
            if side == "SELL" and predicted_edge >= self.SELL_TREND_EDGE_THRESHOLD:
                edge += 0.35

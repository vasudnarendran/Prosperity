...
class TomatoesBot:
    ...
    def load_product_state(self) -> Dict[str, float]:
        ...
        return {
            ...
            "fill_quality_ewma": float(raw.get("fill_quality_ewma", 0.0)),
            "adverse_ewma": float(raw.get("adverse_ewma", 0.0)),
            "last_mid": float(raw.get("last_mid", 0.0)),
            "last_fill_ts": float(raw.get("last_fill_ts", -1.0)),
    ...
    def save_product_state(self) -> None:
        self.memory["tomatoes"] = {
            ...
            "fill_quality_ewma": self.product_state["fill_quality_ewma"],
            "adverse_ewma": self.product_state["adverse_ewma"],
            "last_mid": self.product_state["last_mid"],
            "last_fill_ts": self.product_state["last_fill_ts"],
            "initialized": 1,
        }
    ...
    def update_fill_feedback(self) -> None:
        ...
        pending = self.memory.get("tomatoes_pending_fills", [])
        if not isinstance(pending, list):
            pending = []

        current_ts = int(getattr(self.state, "timestamp", 0))
        matured: List[Dict[str, float]] = []
        still_pending: List[Dict[str, float]] = []
        for item in pending:
            if not isinstance(item, dict):
                continue
            fill_ts = int(item.get("ts", -1))
            if fill_ts < 0:
                continue
            if current_ts - fill_ts >= self.MARKOUT_HORIZON:
                matured.append(item)
            else:
        ...
        for item in matured:
            ...
            fill_price = float(item.get("price", self.book.mid))
            if side == "BUY":
                markout = self.book.mid - fill_price
            else:
                markout = fill_price - self.book.mid
            self.product_state["fill_quality_ewma"] = ema(
                self.product_state["fill_quality_ewma"], markout, self.FEEDBACK_ALPHA
            )
            adverse = 1.0 if markout < -0.4 else 0.0
            self.product_state["adverse_ewma"] = ema(
                self.product_state["adverse_ewma"], adverse, self.FEEDBACK_ALPHA
            )

        last_fill_ts = int(self.product_state["last_fill_ts"])
        own_trades = self.state.own_trades.get(self.product, [])
            ...
            still_pending.append(
                {
                    "ts": float(trade_ts),
                    "side": side,
                    "price": float(getattr(trade, "price", self.book.mid)),
                }
            )
            last_fill_ts = max(last_fill_ts, trade_ts)

        self.product_state["last_fill_ts"] = float(last_fill_ts)
        self.memory["tomatoes_pending_fills"] = still_pending
    ...
    def update_state(self) -> None:
        ...
        fast_fair = (
            0.45 * self.book.mid
            + 0.35 * self.book.micro
            + 0.20 * (self.book.mid + current_flow)
        )
        slow_fair = self.product_state["wall_fair_ema"]
        trend_raw = (
            1.10 * (fast_fair - slow_fair)
    ...
    def in_up_regime(self, regime: str) -> bool:
        return regime in {"trend_up", "strong_up"}

    def in_down_regime(self, regime: str) -> bool:
        return regime in {"trend_down", "strong_down"}
    ...
    def take_threshold(self, side: str, regime: str, target: int) -> float:
        ...
        position = self.manager.projected_position()
        fill_quality = self.product_state["fill_quality_ewma"]
        adverse = self.product_state["adverse_ewma"]
        ...
        if self.in_up_regime(regime) and side == "BUY" and position < 0:
            threshold -= 0.45 if regime == "trend_up" else 0.65
        if self.in_down_regime(regime) and side == "SELL" and position > 0:
            threshold -= 0.45 if regime == "trend_down" else 0.65

        threshold += 0.30 * adverse
        threshold -= 0.08 * clamp(fill_quality, 0.0, 2.5)
    ...
    def clear_inventory(self, reservation: float, regime: str, target: int) -> None:
        position = self.manager.projected_position()
        long_clear_edge = reservation
        short_clear_edge = reservation

        if self.in_up_regime(regime) and position > 0:
            long_clear_edge += 1.0
        elif self.in_down_regime(regime) and position > 0:
            long_clear_edge -= 0.4

        if self.in_down_regime(regime) and position < 0:
            short_clear_edge -= 1.0
        elif self.in_up_regime(regime) and position < 0:
            short_clear_edge += 0.4

        if position > 0 and self.book.best_bid >= math.floor(long_clear_edge):
            size = min(position, self.book.best_bid_volume, self.PASSIVE_SIZE)
            if self.in_up_regime(regime) and position <= max(target, 0):
                size = 0
            self.manager.add_sell(self.book.best_bid, size)

        position = self.manager.projected_position()
        if position < 0 and self.book.best_ask <= math.ceil(short_clear_edge):
            size = min(abs(position), self.book.best_ask_volume, self.PASSIVE_SIZE)
            if self.in_down_regime(regime) and abs(position) <= max(abs(target), 0):
                size = 0
            self.manager.add_buy(self.book.best_ask, size)
    ...
    def quote_edge(self, side: str, regime: str, target: int) -> float:
        ...
        if regime == "trend_up":
            edge += -0.20 if side == "BUY" else 0.30
        elif regime == "strong_up":
            edge += -0.30 if side == "BUY" else 0.45
        elif regime == "trend_down":
            edge += -0.20 if side == "SELL" else 0.30
        elif regime == "strong_down":
            edge += -0.30 if side == "SELL" else 0.45
        ...
        edge += 0.35 * adverse
        edge -= 0.06 * clamp(fill_quality, 0.0, 2.0)
        return max(1.2, edge)
    ...
    def allow_passive(self, side: str, regime: str, target: int) -> bool:
        ...
        if regime == "toxic" and abs(position) <= 4:
            return False
        if regime == "strong_up" and side == "SELL" and position <= max(8, target // 2):
            return False
        if regime == "trend_up" and side == "SELL" and position <= max(0, target // 3):
            return False
        if regime == "strong_down" and side == "BUY" and position >= min(-8, target // 2):
            return False
        if regime == "trend_down" and side == "BUY" and position >= min(0, target // 3):
            return False
        if self.in_up_regime(regime) and side == "SELL" and position < 0:
            return False
        if self.in_down_regime(regime) and side == "BUY" and position > 0:
            return False
        return True

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "SHORT_REGRESSION_HORIZON": 1.0,
    "LONG_REGRESSION_WINDOW": 8,
    "LONG_REGRESSION_HORIZON": 2.5,
    "BUY_TREND_EDGE_THRESHOLD": 1.25,
    "SELL_TREND_EDGE_THRESHOLD": 1.75,
    "STRONG_TREND_EDGE": 3.50,
    "FIT_THRESHOLD": 0.65,
    "PERSISTENCE_CONFIRMATIONS": 2,
    "TREND_IMBALANCE_THRESHOLD": 0.12,
    "TOXIC_SPREAD_THRESHOLD": 15.0,
    "TOXIC_VOLATILITY_THRESHOLD": 3.2,
    "SOFT_LIMIT_RATIO": 0.55,
    "SELL_HOLD_LONG_EDGE": 0.75,
    "SELL_STRONG_HOLD_EDGE": 2.0,
    "SELL_QUOTE_LIFT": 1.0,
    "SELL_TAKE_PENALTY": 0.25,
    "SELL_RELEASE_BUFFER": 6,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def regression_metrics(self, window_size: int, horizon: float) -> Tuple[float, float, float, float, float]:
        history = self.mid_history.get(self.product, [])
        ...
        predicted_next = intercept + slope * ((n - 1) + horizon)
    ...
    def classify_state(
        ...
        if float(self.spread) >= self.TOXIC_SPREAD_THRESHOLD and volatility >= self.TOXIC_VOLATILITY_THRESHOLD:
            return "volatile"
        if (
            short_edge >= self.BUY_TREND_EDGE_THRESHOLD
            and long_edge >= 0.25 * self.BUY_TREND_EDGE_THRESHOLD
            and short_fit >= self.FIT_THRESHOLD
            and long_fit >= 0.55
            and self.imbalance >= self.TREND_IMBALANCE_THRESHOLD
            and self.momentum >= 0.40
            and float(self.micro) >= float(self.mid)
        ):
            return "trend_up"
        if (
            short_edge <= -self.SELL_TREND_EDGE_THRESHOLD
            and long_edge <= -0.25 * self.SELL_TREND_EDGE_THRESHOLD
            and short_fit >= self.FIT_THRESHOLD
            and long_fit >= 0.55
            and self.imbalance <= -self.TREND_IMBALANCE_THRESHOLD
            and self.momentum <= -0.40
    ...
    def persistent_regime(self, proposed_regime: str) -> str:
        memory = self.signal_state.setdefault(
            self.product,
            {
        ...
        effective = str(memory.get("effective_regime", "range"))
        candidate = str(memory.get("candidate_regime", "range"))
        count = int(memory.get("candidate_count", 0))

        if proposed_regime == "volatile":
            effective = "volatile"
            candidate = "volatile"
            count = 0
        elif proposed_regime == "range":
            effective = "range"
            candidate = "range"
            count = 0
        elif proposed_regime == effective:
            candidate = proposed_regime
            count = 0
        else:
            if effective in {"trend_up", "trend_down"} and proposed_regime != effective:
                effective = "range"

            if proposed_regime == candidate:
                count += 1
            else:
                candidate = proposed_regime
                count = 1

            if count >= int(self.PERSISTENCE_CONFIRMATIONS):
                effective = proposed_regime
                candidate = proposed_regime
                count = 0

        memory["effective_regime"] = effective
        memory["candidate_regime"] = candidate
        memory["candidate_count"] = count
        return effective

    def target_band(self, regime: str, short_edge: float, fit_quality: float) -> Tuple[int, int]:
        conviction = abs(short_edge) * max(0.5, fit_quality)
        if regime == "trend_up":
            return (14, 28) if conviction >= self.STRONG_TREND_EDGE else (6, 18)
        if regime == "trend_down":
            return (-28, -14) if conviction >= self.STRONG_TREND_EDGE else (-18, -6)
        if regime == "volatile":
            return -6, 6
        return -10, 10

    def target_position(self, regime: str, short_edge: float, fit_quality: float) -> int:
        lower, upper = self.target_band(regime, short_edge, fit_quality)
        position = self.projected_position()
        if position < lower:
            return lower
        if position > upper:
    ...
    def fair_value(
        ...
        regime: str,
        target_position: int,
        short_predicted_now: float,
        short_predicted_next: float,
        long_edge: float,
    ) -> float:
        line_gap = short_predicted_now - float(self.mid)
        fair = (
            self.MID_WEIGHT * float(self.mid)
            + self.MICRO_WEIGHT * float(self.micro)
            + self.HISTORY_WEIGHT * float(self.recent_average)
            + self.REGRESSION_WEIGHT * short_predicted_next
        ...
        if regime == "range":
            ...
            fair += (0.10 * line_gap) + (0.15 * (short_predicted_next - float(self.mid)))
            if regime == "trend_up" and long_edge > 0:
                fair += 0.08 * min(2.0, long_edge)
            elif regime == "trend_down" and long_edge < 0:
                fair += 0.08 * max(-2.0, long_edge)
        return fair
    ...
    def adjusted_fair_value(
        ...
        fair = self.fair_value(regime, target_position, short_predicted_now, short_predicted_next, long_edge)
    ...
    def take_edge(
        ...
        if regime == "trend_up":
            edge += -0.25 if side == "BUY" else 0.35
        elif regime == "trend_down":
            edge += -0.25 if side == "SELL" else 0.35
        elif regime == "volatile":
            edge += 0.50
        else:
            if side == "BUY" and short_edge <= -self.BUY_TREND_EDGE_THRESHOLD:
                edge += 0.35
            if side == "SELL" and short_edge >= self.SELL_TREND_EDGE_THRESHOLD:
                edge += 0.35

        if short_edge > 0 and side == "BUY":
            edge -= min(0.20, 0.05 * short_edge * max(0.5, fit_quality))
        elif short_edge < 0 and side == "SELL":
            edge -= min(0.18, 0.05 * abs(short_edge) * max(0.5, fit_quality))

        if side == "SELL" and long_edge >= self.SELL_HOLD_LONG_EDGE and short_edge >= 0.25:
            edge += self.SELL_TAKE_PENALTY
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            ...
            if sell_quote is not None and long_edge >= self.SELL_HOLD_LONG_EDGE and short_edge >= 0.25 and position <= target_position + int(self.SELL_RELEASE_BUFFER):
                sell_quote += int(self.SELL_QUOTE_LIFT)
            if sell_quote is not None and long_edge >= self.SELL_STRONG_HOLD_EDGE and short_edge >= 0.50 and position <= 8:
                sell_quote += 1
        elif regime == "trend_down":
            ...
            if buy_quote is not None and short_edge <= -self.STRONG_TREND_EDGE and position >= -8:
                buy_quote -= 1
        elif regime == "range" and sell_quote is not None and long_edge >= self.SELL_HOLD_LONG_EDGE and short_edge >= 0.50 and position <= 8:
            sell_quote += 1
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, short_edge, long_edge, fit_quality, volatility)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, short_edge, long_edge, fit_quality, volatility)
            and self.sell_capacity > 0
        ):
            take_limit = self.MAX_TAKE_SIZE + (2 if regime == "trend_down" else 0)
            position = self.projected_position()
            if regime != "range" and position <= target_position:
                pass
            elif regime == "trend_up" and long_edge >= self.SELL_HOLD_LONG_EDGE and short_edge >= 0.25 and position <= target_position + int(self.SELL_RELEASE_BUFFER):
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        (
            _short_slope,
            short_predicted_now,
            short_predicted_next,
            short_fit,
            short_volatility,
        ) = self.regression_metrics(int(self.SHORT_REGRESSION_WINDOW), float(self.SHORT_REGRESSION_HORIZON))
        (
            _long_slope,
            _long_predicted_now,
            long_predicted_next,
            long_fit,
            long_volatility,
        ) = self.regression_metrics(int(self.LONG_REGRESSION_WINDOW), float(self.LONG_REGRESSION_HORIZON))
        short_edge = short_predicted_next - float(self.mid)
        long_edge = long_predicted_next - float(self.mid)
        fit_quality = max(short_fit, long_fit)
        volatility = max(short_volatility, long_volatility)
        proposed_regime = self.classify_state(short_edge, long_edge, short_fit, long_fit, volatility)
        regime = self.persistent_regime(proposed_regime)
        target_position = self.target_position(regime, short_edge, fit_quality)
        adjusted_fair = self.adjusted_fair_value(regime, target_position, short_predicted_now, short_predicted_next, long_edge)
...
class Trader:
    ...
    def load_trader_data(self, trader_data: str) -> Tuple[Dict[str, List[float]], Dict[str, Dict[str, object]]]:
        ...
        raw_signal_state = parsed.get("signal_state", {})
        ...
        cleaned_signal_state: Dict[str, Dict[str, object]] = {}
        if isinstance(raw_signal_state, dict):
            for product, values in raw_signal_state.items():
                if not isinstance(values, dict):
                    continue
                cleaned_signal_state[product] = {
                    "effective_regime": str(values.get("effective_regime", "range")),
                    "candidate_regime": str(values.get("candidate_regime", "range")),
                    "candidate_count": int(values.get("candidate_count", 0)),
                }

        return cleaned, cleaned_signal_state
    ...
    def build_trader_data(
        ...
        return json.dumps({"mid_history": mid_history, "signal_state": signal_state}, separators=(",", ":"))

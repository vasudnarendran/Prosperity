...
class Trader:
    ...
    def passive_bucket_key(
        ...
        return f"{side}|d{min(4, touch_gap)}|{spread_bucket}|{toxic_bucket}|{inv_bucket}"
    ...
    def level_flow_features(
        ...
        return flow_features[0], flow_features[1], flow_features[2]
    ...
    def taker_raw_features(
        ...
        book: Dict[str, object],
        ml_imbalance: float,
        flow_features: Tuple[float, float, float],
        volatility: float,
    ) -> List[float]:
        ...
        imbalance_curve = float(book["l1_imbalance"]) - ml_imbalance
        return [
            1.0,
            ...
            clip(flow_features[2], -2.0, 2.0),
            clip(short_return, -3.0, 3.0),
            clip(spread_norm, 0.5, 2.0),
            clip(imbalance_curve, -1.5, 1.5),
        ]
    ...
    def maker_raw_features(
        ...
        return [
            ...
            clip(side_sign * flow_features[1], -2.0, 2.0),
            clip(side_sign * flow_features[2], -2.0, 2.0),
    ...
    def regime_weights(
        ...
        flow_score = flow_features[0] + 0.60 * flow_features[1] + 0.35 * flow_features[2]
    ...
    def trade_tomatoes(
        ...
        target_position = self.target_position(regime, taker_alpha, stretch, soft_limit, volatility, position)
        ...
        buy_priority = self.aggressive_priority("BUY", book, float(book["mid"]) + taker_alpha, buy_edge)
        sell_priority = self.aggressive_priority("SELL", book, float(book["mid"]) + taker_alpha, sell_edge)
        ...
        for side in (first_side, second_side):
            ...
            self.sweep_book(side, book, builder, float(book["mid"]) + taker_alpha, edge, target_position, soft_limit)

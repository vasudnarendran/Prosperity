...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MAKER_JOIN_THRESHOLD": 1.0,
    "CLEAN_SPREAD_PASSIVE_BONUS": 1.0,
    "EXIT_HOLD_BONUS": 0.20,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def passive_quotes(
        ...
        if inside_buy is not None and buy_quote is not None and inside_buy >= buy_quote - self.MAKER_JOIN_THRESHOLD:
            buy_quote = max(buy_quote, inside_buy)
        if inside_sell is not None and sell_quote is not None and inside_sell <= sell_quote + self.MAKER_JOIN_THRESHOLD:
        ...
        if regime == "trend_up":
            ...
            if sell_quote is not None and predicted_edge >= self.STRONG_TREND_EDGE and position <= 8:
                sell_quote += 1
        elif regime == "trend_down":
            ...
            if buy_quote is not None and predicted_edge <= -self.STRONG_TREND_EDGE and position >= -8:
                buy_quote -= 1
    ...
    def take_orders(
        ...
        if (
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif regime == "trend_up" and predicted_edge >= self.TREND_EDGE_THRESHOLD and sell_dislocation < self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility) + self.EXIT_HOLD_BONUS:
                pass
            else:

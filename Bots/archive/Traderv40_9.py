...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_FAIR_EWMA_ALPHA": 0.18,
    "TREND_FAIR_DRIFT_WEIGHT": 1.10,
    "RESIDUAL_REVERT_SCALE": 0.70,
    "RESIDUAL_OVERSTRETCH_THRESHOLD": 0.95,
    "RESIDUAL_BRAKE_STRENGTH": 0.18,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.trend_fair = float(self.memory.get("trend_fair", self.mid or 0.0))
        self.residual_ewma = float(self.memory.get("residual_ewma", 0.0))
    ...
    def update_state(self) -> None:
        ...
        trend_alpha = self.TREND_FAIR_EWMA_ALPHA
        mid = float(self.mid)
        ...
        trend_target = self.ewma_mid + (self.TREND_FAIR_DRIFT_WEIGHT * self.ewma_return)
        self.trend_fair = (1.0 - trend_alpha) * self.trend_fair + trend_alpha * trend_target
        residual = mid - self.trend_fair
        self.residual_ewma = (1.0 - alpha) * self.residual_ewma + alpha * residual
        self.prev_mid = mid

    def detrended_residual(self, volatility: float) -> float:
        return self.residual_ewma / max(1.0, volatility)
    ...
    def guarded_hybrid_alpha(self, hybrid_alpha: float, regression_edge: float, regime: str) -> float:
        weight = 1.0
        residual_sign = self.residual_ewma
        if regime == "range":
        ...
        if hybrid_alpha * residual_sign > 0:
            weight *= 1.0 - min(0.35, self.RESIDUAL_BRAKE_STRENGTH * abs(residual_sign) / max(1.0, float(self.spread)))
    ...
    def classify_state(
        ...
        residual = self.detrended_residual(volatility)
        ...
        if (
            ...
            and residual <= self.RESIDUAL_OVERSTRETCH_THRESHOLD
        ):
            return "trend_up"
        if (
            predicted_edge <= -trend_threshold
    ...
    def fair_value(
        ...
        residual_gap = self.trend_fair - float(self.mid)
        ...
        if regime == "range":
            fair += self.RESIDUAL_REVERT_WEIGHT * (
                (1.0 - self.RESIDUAL_REVERT_SCALE) * line_gap
                + self.RESIDUAL_REVERT_SCALE * residual_gap
            )
        else:
    ...
    def take_edge(
        ...
        if side == "BUY" and predicted_edge > 0 and residual > self.RESIDUAL_OVERSTRETCH_THRESHOLD:
            edge += self.RESIDUAL_BRAKE_STRENGTH * min(2.0, residual - self.RESIDUAL_OVERSTRETCH_THRESHOLD)
        elif side == "SELL" and predicted_edge < 0 and residual < -self.RESIDUAL_OVERSTRETCH_THRESHOLD:
            edge += self.RESIDUAL_BRAKE_STRENGTH * min(2.0, abs(residual) - self.RESIDUAL_OVERSTRETCH_THRESHOLD)
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            if buy_quote is not None and position < target_position:
                ...
                if residual > self.RESIDUAL_OVERSTRETCH_THRESHOLD:
                    buy_quote -= 1
    ...
    def run(self) -> List[Order]:
        ...
        self.memory = {
            ...
            "trend_fair": round(self.trend_fair, 4),
            "residual_ewma": round(self.residual_ewma, 4),
            "mode": self.mode,

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "CONFIRMED_TREND_MOMENTUM": 2.2,
    "CONFIRMED_SHORT_MOMENTUM": 0.8,
    "CONFIRMED_IMBALANCE": 0.20,
    "CONFIRMED_TREND_FAIR_BONUS": 0.70,
    "CONFIRMED_TREND_EDGE_NUDGE": 0.15,
    "CONFIRMED_TREND_QUOTE_NUDGE": 1.0,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def confirmed_trend(self, regime: str) -> str:
        if (
            regime == "trend_up"
            and self.momentum >= self.CONFIRMED_TREND_MOMENTUM
            and self.short_momentum >= self.CONFIRMED_SHORT_MOMENTUM
            and self.imbalance >= self.CONFIRMED_IMBALANCE
        ):
            return "trend_up"
        if (
            regime == "trend_down"
    ...
    def adjusted_fair_value(self, target_position: int, confirmed_trend: str) -> float:
        fair = self.fair_value(target_position)
        if confirmed_trend == "trend_up":
            fair += self.CONFIRMED_TREND_FAIR_BONUS + 0.20 * max(0.0, self.short_momentum)
        elif confirmed_trend == "trend_down":
            fair -= self.CONFIRMED_TREND_FAIR_BONUS + 0.20 * max(0.0, -self.short_momentum)
        return fair - (self.projected_position() * self.INVENTORY_SKEW)

    def take_edge(self, side: str, regime: str, confirmed_trend: str) -> float:
        ...
        if confirmed_trend == "trend_up":
            if side == "BUY":
                edge -= self.CONFIRMED_TREND_EDGE_NUDGE
            else:
                edge += self.CONFIRMED_TREND_EDGE_NUDGE
        elif confirmed_trend == "trend_down":
            if side == "SELL":
    ...
    def passive_quotes(
        ...
        if confirmed_trend == "trend_up" and sell_quote is not None:
            sell_quote += int(self.CONFIRMED_TREND_QUOTE_NUDGE)
        elif confirmed_trend == "trend_down" and buy_quote is not None:
            buy_quote -= int(self.CONFIRMED_TREND_QUOTE_NUDGE)
    ...
    def take_orders(
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, confirmed_trend) and self.buy_capacity > 0:
        ...
        if int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, confirmed_trend) and self.sell_capacity > 0:
    ...
    def run(self) -> List[Order]:
        ...
        confirmed_trend = self.confirmed_trend(regime)
        target_position = self.target_position(regime)
        adjusted_fair = self.adjusted_fair_value(target_position, confirmed_trend)
        took_buy, took_sell = self.take_orders(regime, target_position, adjusted_fair, confirmed_trend)

        buy_quote, sell_quote = self.passive_quotes(adjusted_fair, regime, target_position, confirmed_trend)
        position = self.projected_position()

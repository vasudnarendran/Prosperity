...
class Trader:
    ...
    def get_bullish_sell_restraint(
        self,
        product: str,
        position: int,
        market_state: Dict[str, float],
        ...
        restraint = 0.0
        if market_state["momentum"] >= self.TOMATOES_BULLISH_MOMENTUM:
            restraint += 0.5
        if market_state["momentum"] >= self.TOMATOES_STRONG_BULLISH_MOMENTUM:
            restraint += 0.5
        if market_state["imbalance"] >= self.TOMATOES_BULLISH_IMBALANCE:
            restraint += 0.5
    ...
    def get_take_edge(
        ...
        if product == "TOMATOES":
            bullish_sell_restraint = self.get_bullish_sell_restraint(
                product,
                position,
                market_state,
            )

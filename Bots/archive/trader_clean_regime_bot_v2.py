...
TOMATOES_CONFIG = {
    ...
    "ALPHA_DISPLACEMENT_WEIGHT": 0.80,
    "BREAKOUT_SCORE_BONUS": 0.35,
    "REVERSAL_THRESHOLD_BONUS": 1.25,
    "SECOND_LAYER_SPREAD_THRESHOLD": 6,
...
class TomatoesTrader(BaseProductTrader):
    def __init__(
        ...
        self.ewma_slow_mid = float(self.memory.get("ewma_slow_mid", self.mid or 0.0))
    ...
    def update_state(self) -> None:
        ...
        slow_alpha = self.SLOW_EWMA_ALPHA
        mid = float(self.mid)
        micro_gap = float(self.micro) - mid
        ret = mid - self.prev_mid

        self.ewma_mid = (1.0 - alpha) * self.ewma_mid + alpha * mid
        self.ewma_slow_mid = (1.0 - slow_alpha) * self.ewma_slow_mid + slow_alpha * mid
    ...
    def displacement(self) -> float:
        return self.ewma_mid - self.ewma_slow_mid

    def normalized_displacement(self) -> float:
        scale = max(1.0, 1.5 * self.ewma_abs_return)
        return self.displacement() / scale
    ...
    def update_fill_feedback(self) -> None:
        ...
        for fill in self.pending_fills:
            ...
            alpha = clamp(self.FILL_EWMA_ALPHA * weight, 0.05, 0.35)

            self.fill_quality = (1.0 - alpha) * self.fill_quality + alpha * scaled_markout
            adverse_obs = 1.0 if signed_markout < -0.5 else 0.0
            self.adverse = (1.0 - alpha) * self.adverse + alpha * adverse_obs
    ...
    def alpha_score(self) -> float:
        scale = max(0.75, self.ewma_abs_return)
        displacement = self.normalized_displacement()
        raw = (
            ...
            + self.ALPHA_DISPLACEMENT_WEIGHT * displacement
        )

        if self.ewma_return * displacement > 0.0:
            raw += self.BREAKOUT_SCORE_BONUS * displacement

        blend = 0.34 if raw * self.regime_score < 0.0 else 0.22
        self.regime_score = (1.0 - blend) * self.regime_score + blend * raw
        return clamp(self.regime_score, -3.5, 3.5)

    def classify_mode(self, score: float) -> str:
        prev = self.mode
        disp = self.normalized_displacement()
        breakout_up = (
            disp >= 0.80
            and self.ewma_return > 0.18
            and self.ewma_micro_gap > -0.05
        )
        breakout_down = (
            disp <= -0.80
            and self.ewma_return < -0.18
        ...
        if breakout_up and prev in {"lean_down", "strong_down"}:
            return "lean_up" if score < enter_strong else "strong_up"
        if breakout_down and prev in {"lean_up", "strong_up"}:
            return "lean_down" if score > -enter_strong else "strong_down"

        if prev == "strong_up":
            if score >= exit_strong and not breakout_down:
                return "strong_up"
        elif prev == "lean_up":
            if score >= exit_lean and not breakout_down:
                return "lean_up"
        elif prev == "strong_down":
            if score <= -exit_strong and not breakout_up:
                return "strong_down"
        elif prev == "lean_down":
            if score <= -exit_lean and not breakout_up:
                return "lean_down"

        if breakout_up and score >= 0.0:
            return "strong_up" if score >= enter_strong else "lean_up"
        if breakout_down and score <= 0.0:
            return "strong_down" if score <= -enter_strong else "lean_down"
    ...
    def base_fair_value(self, score: float, mode: str) -> float:
        ...
        fair = (
            0.32 * float(self.mid)
            + 0.22 * self.ewma_mid
            + 0.18 * self.ewma_slow_mid
            + 0.18 * float(self.micro)
            + 0.05 * flow_term
            + 0.05 * trend_term
        )

        if mode in {"lean_up", "strong_up"}:
            fair += 0.28 * max(0.0, disp) + 0.16 * max(0.0, self.ewma_micro_gap)
        elif mode in {"lean_down", "strong_down"}:
            fair += 0.28 * min(0.0, disp) + 0.16 * min(0.0, self.ewma_micro_gap)

        return fair

    def reservation_price(self, target_position: int, score: float, mode: str) -> float:
        fair = self.base_fair_value(score, mode)
        pos = self.projected_position()
        ...
        reservation += 0.20 * self.ewma_return
        reservation += 0.12 * self.ewma_micro_gap
        return reservation

    def reversal_pressure(self, side: str, mode: str, target_position: int) -> float:
        pos = self.projected_position()
        if side == "BUY" and mode in {"lean_up", "strong_up"} and pos < 0:
            return self.REVERSAL_THRESHOLD_BONUS + 0.25 * min(2.0, abs(pos) / max(1, self.soft_limit))
        if side == "SELL" and mode in {"lean_down", "strong_down"} and pos > 0:
            return self.REVERSAL_THRESHOLD_BONUS + 0.25 * min(2.0, abs(pos) / max(1, self.soft_limit))
        return 0.0
    ...
    def quote_edge(self, side: str, mode: str) -> float:
        ...
        if favorable_buy or favorable_sell:
            edge -= 0.45 if "strong" in mode else 0.20
        if unfavorable_buy or unfavorable_sell:
            edge += 1.00 if "strong" in mode else 0.55
    ...
    def layered_passive_quotes(self, reservation: float, mode: str, target_position: int) -> None:
        ...
        second_layer_ok = int(self.spread) >= self.SECOND_LAYER_SPREAD_THRESHOLD and mode in {"neutral", "lean_up", "lean_down"}
        ...
        if self.allow_side("BUY", mode, target_position) and self.buy_capacity > 0:
            ...
            if second_layer_ok and self.buy_capacity > 0:
        ...
        if self.allow_side("SELL", mode, target_position) and self.sell_capacity > 0:
            ...
            if second_layer_ok and self.sell_capacity > 0:
    ...
    def aggressive_logic(self, reservation: float, mode: str, target_position: int) -> None:
        ...
        buy_threshold = reservation - self.take_edge("BUY", mode)
        sell_threshold = reservation + self.take_edge("SELL", mode)
        buy_levels = 3
        sell_levels = 3

        reversal_buy = self.reversal_pressure("BUY", mode, target_position)
        reversal_sell = self.reversal_pressure("SELL", mode, target_position)

        if reversal_buy > 0.0:
            buy_threshold += reversal_buy
            buy_qty = max(buy_qty, min(self.MAX_TAKE_SIZE + 4, abs(pos) + max(0, target_position)))
            buy_levels = 4 if mode == "strong_up" else 3

        if reversal_sell > 0.0:
            sell_threshold -= reversal_sell
            sell_qty = max(sell_qty, min(self.MAX_TAKE_SIZE + 4, abs(pos) + max(0, -target_position)))
            sell_levels = 4 if mode == "strong_down" else 3

        if self.allow_side("BUY", mode, target_position) and buy_qty > 0:
            if self.fill_quality > 0.25 and self.adverse < 0.35:
                buy_qty += 2
            self.sweep_book("BUY", buy_threshold, buy_qty, max_levels=buy_levels)

        if self.allow_side("SELL", mode, target_position) and sell_qty > 0:
            if self.fill_quality > 0.25 and self.adverse < 0.35:
                sell_qty += 2
            self.sweep_book("SELL", sell_threshold, sell_qty, max_levels=sell_levels)
    ...
    def run(self) -> List[Order]:
        ...
        reservation = self.reservation_price(target, score, mode)
        ...
        self.memory = {
            ...
            "ewma_slow_mid": round(self.ewma_slow_mid, 4),

...
class TomatoesTrader(BaseProductTrader):
    ...
    def regression_metrics(self, window_size: Optional[int] = None, horizon: Optional[float] = None) -> Tuple[float, float, float, float, float]:
        history = self.mid_history.get(self.product, [])
        window = history[-int(window_size or self.REGRESSION_WINDOW) :]
        if len(window) < 2:
        ...
        predicted_next = intercept + slope * ((n - 1) + float(horizon or self.REGRESSION_HORIZON))
    ...
    def fair_value(
        ...
        fair = (
            ...
            + self.LONG_REGRESSION_WEIGHT * long_predicted_next
    ...
    def adjusted_fair_value(
        ...
        fair = self.fair_value(regime, target_position, predicted_now, predicted_next, long_predicted_next)
    ...
    def run(self) -> List[Order]:
        ...
        _long_slope, _long_now, long_predicted_next, _long_fit, _long_vol = self.regression_metrics(
            int(self.LONG_REGRESSION_WINDOW),
            float(self.LONG_REGRESSION_HORIZON),
        )
        ...
        adjusted_fair = self.adjusted_fair_value(regime, target_position, predicted_now, predicted_next, long_predicted_next)

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "PRESSURE_MICRO_COEF": 1.00,
    "PRESSURE_SHORT_MOMENTUM_COEF": 0.35,
    "PRESSURE_IMBALANCE_COEF": 1.80,
    "PRESSURE_FAIR_BONUS": 0.80,
    "PRESSURE_EDGE_BONUS": 0.20,
    "PRESSURE_QUOTE_THRESHOLD": 0.65,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def pressure_signal(self) -> float:
        ...
        micro_bias = (float(self.micro) - float(self.mid)) / spread_scale
        raw = (
            self.PRESSURE_MICRO_COEF * micro_bias
            + self.PRESSURE_SHORT_MOMENTUM_COEF * self.short_momentum
            + self.PRESSURE_IMBALANCE_COEF * self.imbalance
        )
        return max(-2.0, min(2.0, raw))

    def fair_value(self, target_position: int, pressure: float) -> float:
        fair = (
            self.MID_WEIGHT * float(self.mid)
            + self.MICRO_WEIGHT * float(self.micro)
        ...
        fair += pressure * self.PRESSURE_FAIR_BONUS
        return fair

    def adjusted_fair_value(self, target_position: int, pressure: float) -> float:
        return self.fair_value(target_position, pressure) - (self.projected_position() * self.INVENTORY_SKEW)

    def take_edge(self, side: str, regime: str, pressure: float) -> float:
        ...
        if pressure > 0:
            if side == "BUY":
                edge -= self.PRESSURE_EDGE_BONUS * pressure
            else:
                edge += 0.6 * self.PRESSURE_EDGE_BONUS * pressure
        elif pressure < 0:
            if side == "SELL":
                edge -= self.PRESSURE_EDGE_BONUS * abs(pressure)
            else:
                edge += 0.6 * self.PRESSURE_EDGE_BONUS * abs(pressure)
    ...
    def passive_quotes(
        ...
        if pressure >= self.PRESSURE_QUOTE_THRESHOLD:
            if buy_quote is not None and position < target_position:
                buy_quote += 1
            if sell_quote is not None and regime != "mean_revert":
                sell_quote += 1
        elif pressure <= -self.PRESSURE_QUOTE_THRESHOLD:
            if sell_quote is not None and position > target_position:
                sell_quote -= 1
            if buy_quote is not None and regime != "mean_revert":
                buy_quote -= 1
    ...
    def take_orders(
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, pressure) and self.buy_capacity > 0:
        ...
        if int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, pressure) and self.sell_capacity > 0:
    ...
    def run(self) -> List[Order]:
        ...
        adjusted_fair = self.adjusted_fair_value(target_position, pressure)
        took_buy, took_sell = self.take_orders(regime, target_position, adjusted_fair, pressure)

        buy_quote, sell_quote = self.passive_quotes(adjusted_fair, regime, target_position, pressure)
        position = self.projected_position()

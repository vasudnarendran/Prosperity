...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "HOLD_TIME_COEF": 0.08,
    "HOLD_VOL_COEF": 0.0,
    "REBOUND_EXIT_TICKS": 2.0,
    "REBOUND_STOP_TICKS": 7.0,
    "REBOUND_MAX_HOLD_TICKS": 2600.0,
    "REBOUND_RELEASE_EDGE": 0.75,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.apply_parameter_overrides(self.PARAMETER_DEFAULTS, params)
        self.soft_limit = int(position_limit * self.SOFT_LIMIT_RATIO)
        self.memory: Dict[str, float] = dict(memory or {})
        self.rebound_state: Dict[str, float] = {
            "active": float(self.memory.get("active", 0.0)),
            "entry_price": float(self.memory.get("entry_price", 0.0)),
            "entry_timestamp": float(self.memory.get("entry_timestamp", 0.0)),
        }

    def update_rebound_entry(self, regime: str, predicted_edge: float) -> None:
        previous_position = int(self.memory.get("position", self.position))
        previous_regime = str(self.memory.get("regime", "range"))
        if self.position <= 0:
            self.rebound_state["active"] = 0.0
            return

        if self.rebound_state["active"] >= 0.5:
            return

        if (
            self.position > previous_position
            and (previous_regime == "trend_down" or regime == "trend_down")
        ):
            entry_price = float(self.best_ask)
            self.rebound_state["active"] = 1.0
            self.rebound_state["entry_price"] = entry_price
            self.rebound_state["entry_timestamp"] = float(getattr(self.state, "timestamp", 0))
            return
    ...
    def rebound_gate_active(self, regime: str, predicted_edge: float) -> bool:
        if self.position <= 0 or self.rebound_state["active"] < 0.5:
            return False

        current_mid = float(self.mid)
        entry_price = self.rebound_state["entry_price"]
        elapsed = float(getattr(self.state, "timestamp", 0)) - self.rebound_state["entry_timestamp"]

        if current_mid >= entry_price + self.REBOUND_EXIT_TICKS:
            self.rebound_state["active"] = 0.0
            return False
        if current_mid <= entry_price - self.REBOUND_STOP_TICKS:
            self.rebound_state["active"] = 0.0
            return False
        if elapsed >= self.REBOUND_MAX_HOLD_TICKS:
            self.rebound_state["active"] = 0.0
            return False
        if predicted_edge >= self.REBOUND_RELEASE_EDGE:
            self.rebound_state["active"] = 0.0
            return False
        if regime == "trend_up":
            self.rebound_state["active"] = 0.0
    ...
    def snapshot_memory(self, regime: str) -> Dict[str, float]:
        return {
            "position": float(self.position),
            "regime": regime,
            "mid": float(self.mid),
            "best_ask": float(self.best_ask),
            "active": float(self.rebound_state["active"]),
            "entry_price": float(self.rebound_state["entry_price"]),
            "entry_timestamp": float(self.rebound_state["entry_timestamp"]),
        }
    ...
    def passive_quotes(
        ...
        if rebound_hold and position > 0:
            sell_quote = None
    ...
    def run(self) -> List[Order]:
        ...
        self.update_rebound_entry(regime, predicted_edge)
        ...
        rebound_hold = self.rebound_gate_active(regime, predicted_edge)
...
class Trader:
    ...
    def load_trader_data(self, trader_data: str) -> Tuple[Dict[str, List[float]], Dict[str, Dict[str, float]]]:
        if not trader_data:
            return {}, {}
        try:
        ...
        cleaned_memory: Dict[str, Dict[str, float]] = {}
        if isinstance(raw_memory, dict):
            for product, values in raw_memory.items():
                if isinstance(values, dict):
                    cleaned_memory[product] = {
    ...
    def build_trader_data(
        self,
        mid_history: Dict[str, List[float]],
        product_memory: Dict[str, Dict[str, float]],
    ) -> str:
        return json.dumps(
    ...
    def run(self, state: TradingState):
        ...
        next_product_memory: Dict[str, Dict[str, float]] = {}
        ...
        for product in state.order_depths:
            ...
            if product == "TOMATOES":
                trader = trader_class(
                    ...
                    memory=product_memory.get(product, {}),
                )
            else:
                trader = trader_class(
                    product,
            ...
            if product == "TOMATOES":
                next_product_memory[product] = trader.snapshot_memory(
                    getattr(trader, "last_regime", "range")
                )

        conversions = 0
        trader_data = self.build_trader_data(mid_history, next_product_memory)

...
class TomatoesBot:
    ...
    def take_threshold(self, side: str, regime: str, target: int) -> float:
        ...
        if regime == "stable":
            ...
            threshold += -0.48 if side == "BUY" else 0.50
        elif regime == "trend_down":
            threshold += -0.48 if side == "SELL" else 0.50
        elif regime == "strong_up":
            threshold += -0.70 if side == "BUY" else 0.70
        elif regime == "strong_down":
            threshold += -0.70 if side == "SELL" else 0.70
        else:
            threshold += 0.65
    ...
    def quote_edge(self, side: str, regime: str, target: int) -> float:
        ...
        if regime == "trend_up":
            edge += -0.18 if side == "BUY" else 0.18
        elif regime == "strong_up":
            edge += -0.30 if side == "BUY" else 0.30
        elif regime == "trend_down":
            edge += -0.18 if side == "SELL" else 0.18
        elif regime == "strong_down":
            edge += -0.30 if side == "SELL" else 0.30

...
class TomatoesBot:
    ...
    def strong_wrong_way_cover(self, side: str, regime: str) -> bool:
        position = self.manager.projected_position()
        if side == "BUY":
            return regime == "strong_up" and position <= -8
        return regime == "strong_down" and position >= 8
    ...
    def take_threshold(self, side: str, regime: str, target: int) -> float:
        ...
        if side == "SELL" and position > target:
            threshold -= 0.10
        if self.strong_wrong_way_cover(side, regime):
            threshold -= 0.60
    ...
    def take_size(self, side: str, regime: str, target: int) -> int:
        ...
        if side == "SELL" and target > self.manager.projected_position():
            size = max(2, size - 3)
        if side == "BUY" and regime == "strong_up" and position < 0:
            size += min(4, max(0, abs(position) - 7))
        if side == "SELL" and regime == "strong_down" and position > 0:
            size += min(4, max(0, abs(position) - 7))
        return min(self.MAX_TAKE_SIZE, size)

    def clear_inventory(self, reservation: float, regime: str) -> None:
        position = self.manager.projected_position()
        long_clear_edge = reservation + (1.0 if regime == "strong_up" and position > 0 else 0.0)
        short_clear_edge = reservation - (1.0 if regime == "strong_down" and position < 0 else 0.0)
    ...
    def allow_passive(self, side: str, regime: str, target: int) -> bool:
        ...
        if regime == "trend_down" and side == "BUY" and position >= -4:
            return False
        if regime == "strong_up" and side == "SELL" and position < 0:
            return False
        if regime == "strong_down" and side == "BUY" and position > 0:
            return False
        return True
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        self.clear_inventory(self.reservation(fair, target), regime)
        reservation = self.reservation(fair, target)

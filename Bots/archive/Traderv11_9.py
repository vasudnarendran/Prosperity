...
class EmeraldsTrader(BaseProductTrader):
    ...
    def run(self) -> List[Order]:
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY") and self.buy_capacity > 0:
            ...
            buy_size = min(self.best_ask_volume, take_limit)
            if int(self.best_ask) == 10000:
                buy_size = min(buy_size, 3)
            self.add_buy(int(self.best_ask), buy_size)
        ...
        if (
            ...
            passive_buy_size = self.passive_size("BUY")
            if buy_quote == 10000:
                passive_buy_size = min(passive_buy_size, 2)
            self.add_buy(buy_quote, passive_buy_size)

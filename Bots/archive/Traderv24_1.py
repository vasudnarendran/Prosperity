...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_EDGE_THRESHOLD": 1.10,
    "STRONG_TREND_EDGE": 2.75,
    "FIT_THRESHOLD": 0.45,
    ...
    "TREND_FAIR_BONUS": 0.18,
    "ALPHA_POSITION_SCALE": 10.0,
    "MAX_SIGNAL_POSITION": 28.0,
    "ALPHA_QUOTE_SKEW": 0.35,
    "ALPHA_TAKE_SKEW": 0.08,
    "JOIN_EDGE": 1.0,
    "DISREGARD_EDGE": 1.5,
    "VOLATILE_SIZE_PENALTY": 3.0,
    "TREND_JOIN_GAP_THRESHOLD": 8.0,
    "RANGE_JOIN_BUFFER": 0.5,
    "TREND_PASSIVE_PENALTY": 3.0,
    "RANGE_PASSIVE_BONUS": 1.0,
    "TREND_EXIT_HOLD_BONUS": 1.00,
    "STRONG_TREND_EXIT_HOLD_BONUS": 0.75,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        conviction = predicted_edge * max(0.5, fit_quality)
        target = int(round(conviction * self.ALPHA_POSITION_SCALE))
        cap = int(self.MAX_SIGNAL_POSITION)
        if regime == "volatile":
            cap = min(cap, 10)
        elif regime == "range":
            cap = min(cap, 18)
    ...
    def position_gap(self, target_position: int) -> int:
        return target_position - self.projected_position()
    ...
    def take_edge(
        ...
        if regime == "trend_up":
            edge += -0.15 if side == "BUY" else 0.25
        elif regime == "trend_down":
            edge += -0.15 if side == "SELL" else 0.25
        ...
        if predicted_edge > 0 and side == "BUY":
            edge -= min(0.25, self.ALPHA_TAKE_SKEW * predicted_edge * max(0.5, fit_quality))
        elif predicted_edge < 0 and side == "SELL":
            edge -= min(0.25, self.ALPHA_TAKE_SKEW * abs(predicted_edge) * max(0.5, fit_quality))
    ...
    def passive_quotes(
        ...
        edge = self.quote_edge(regime, volatility, fit_quality)
        alpha_shift = max(-2, min(2, int(round(predicted_edge * self.ALPHA_QUOTE_SKEW))))
        min_buy_price = math.floor(adjusted_fair - self.JOIN_EDGE)
        min_sell_price = math.ceil(adjusted_fair + self.JOIN_EDGE)
        gap = self.position_gap(target_position)
        position = self.projected_position()

        base_buy = math.floor(adjusted_fair - edge + alpha_shift)
        base_sell = math.ceil(adjusted_fair + edge + alpha_shift)

        join_buy = int(self.best_bid) + 1
        join_sell = int(self.best_ask) - 1

        buy_quote = base_buy
        sell_quote = base_sell

        allow_join_buy = False
        if regime == "range":
            allow_join_buy = join_buy <= math.floor(adjusted_fair - self.RANGE_JOIN_BUFFER)
        elif regime == "trend_up":
            allow_join_buy = (
                gap >= self.TREND_JOIN_GAP_THRESHOLD
                and predicted_edge >= self.STRONG_TREND_EDGE
                and float(self.micro) >= float(self.mid)
            )
        elif regime == "trend_down":
            allow_join_buy = position < 0 and join_buy <= min_buy_price

        if allow_join_buy and join_buy < int(self.best_ask) and join_buy <= min_buy_price:
            buy_quote = max(base_buy, join_buy)

        allow_join_sell = False
        if regime == "range":
            allow_join_sell = join_sell >= math.ceil(adjusted_fair + self.RANGE_JOIN_BUFFER)
        elif regime == "trend_down":
            allow_join_sell = (
                gap <= -self.TREND_JOIN_GAP_THRESHOLD
                and predicted_edge <= -self.STRONG_TREND_EDGE
            ...
            allow_join_sell = position > 0 and join_sell >= min_sell_price

        if allow_join_sell and join_sell > int(self.best_bid) and join_sell >= min_sell_price:
            sell_quote = min(base_sell, join_sell)
        ...
        if regime == "trend_up":
            if gap <= 0 and sell_quote is not None:
                sell_quote += 2
            if gap < self.TREND_JOIN_GAP_THRESHOLD and buy_quote is not None:
                buy_quote = min(buy_quote, math.floor(adjusted_fair - max(2.0, edge)))
        elif regime == "trend_down":
            if gap >= 0 and buy_quote is not None:
                buy_quote -= 2
            if gap > -self.TREND_JOIN_GAP_THRESHOLD and sell_quote is not None:
                sell_quote = max(sell_quote, math.ceil(adjusted_fair + max(2.0, edge)))
    ...
    def passive_size(self, side: str, regime: str, volatility: float, target_position: int) -> int:
        size = self.PASSIVE_SIZE
        if int(self.spread) >= 14:
            size += 1

        if regime == "volatile":
            size = max(1, size - int(self.VOLATILE_SIZE_PENALTY))
        elif regime in {"trend_up", "trend_down"}:
            size = max(1, size - int(self.TREND_PASSIVE_PENALTY))
        elif regime == "range":
            size += int(self.RANGE_PASSIVE_BONUS)
    ...
    def allow_passive(self, side: str, regime: str, target_position: int) -> bool:
        ...
        if regime == "volatile" and abs(position) <= 6:
            return False
        if regime == "trend_up" and side == "SELL" and position <= 0:
            return False
        if regime == "trend_down" and side == "BUY" and position >= 0:
            return False
        if regime == "trend_up" and side == "BUY" and gap <= 0:
            return False
        if regime == "trend_down" and side == "SELL" and gap >= 0:
            return False
        return True
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - buy_edge
            and self.buy_capacity > 0
        ):
            take_limit = self.MAX_TAKE_SIZE
            if regime == "range" or gap > 0:
                quantity = min(self.best_ask_volume, take_limit)
                if regime != "range":
                    quantity = min(quantity, max(1, gap))
                before = self.buy_capacity
                self.add_buy(int(self.best_ask), quantity)
                took_buy = self.buy_capacity < before

        if (
            int(self.best_bid) >= adjusted_fair + sell_edge
            and self.sell_capacity > 0
        ):
            take_limit = self.MAX_TAKE_SIZE
            required_bonus = 0.0
            if regime == "trend_up" and predicted_edge >= self.TREND_EDGE_THRESHOLD:
                required_bonus += self.TREND_EXIT_HOLD_BONUS
            if regime == "trend_up" and predicted_edge >= self.STRONG_TREND_EDGE:
                required_bonus += self.STRONG_TREND_EXIT_HOLD_BONUS
            if regime == "range" or gap < 0:
                if int(self.best_bid) >= adjusted_fair + sell_edge + required_bonus:
    ...
    def run(self) -> List[Order]:
        ...
        if (
            ...
            position_gap = self.position_gap(target_position)
            if regime == "range" or position_gap > 0:
                quantity = min(
                    self.passive_size("BUY", regime, volatility, target_position),
                    self.buy_capacity,
                )
                if regime != "range":
                    quantity = min(quantity, max(1, position_gap))
        ...
        if (
            ...
            if regime == "range" or position_gap < 0:
                quantity = min(
                    self.passive_size("SELL", regime, volatility, target_position),
                    self.sell_capacity,
                )
                if regime != "range":
                    quantity = min(quantity, max(1, -position_gap))

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "FAST_MICRO_BONUS": 0.08,
    "FAST_IMBALANCE_BONUS": 0.18,
    "FAST_ALPHA_BONUS": 0.10,
    "FAST_TAKE_EDGE_BONUS": 0.14,
    "FAST_TAKE_SIZE_BONUS": 2.0,
    "FAST_TARGET_BONUS": 4.0,
    "CALM_JOIN_IMPROVEMENT": 1.0,
    "CALM_QUOTE_EDGE_MULT": 0.94,
    "FAST_QUOTE_EDGE_MULT": 1.08,
    "INVENTORY_EDGE_PRESSURE": 0.55,
    "INVENTORY_SIZE_PRESSURE": 0.45,
    "MARKOUT_DELAY_TICKS": 400,
    "FEEDBACK_EWMA_ALPHA": 0.18,
    "GOOD_FILL_TAKE_BONUS": 0.06,
    "BAD_FILL_TAKE_PENALTY": 0.20,
    "BAD_FILL_QUOTE_PENALTY": 0.22,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def market_style(
        ...
        if calm:
            ...
            if abs(self.projected_position()) <= max(6, int(0.25 * self.soft_limit)) and self.adverse_ewma < 0.35:
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float, style: Dict[str, float]) -> int:
    ...
    def run(self) -> List[Order]:
        ...
        target_position = self.target_position(regime, predicted_edge, fit_quality, style)
        adjusted_fair = self.adjusted_fair_value(
            regime,
            target_position,
            predicted_now,

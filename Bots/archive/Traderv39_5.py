...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "WALL_IMBALANCE_DAMP": 0.58,
    "WALL_MOMENTUM_DAMP": 0.72,
    "WALL_EXTREME_GAP_MULT": 0.95,
    "WALL_EXTREME_DAMP": 0.70,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def blended_wall_fair(self) -> Tuple[Optional[float], float]:
        ...
        strength = min(1.0, self.wall_strength_ewma)
        wall_gap = self.wall_fair_ewma - float(self.mid)

        if wall_gap * self.imbalance < 0:
            strength *= self.WALL_IMBALANCE_DAMP
        if wall_gap * self.momentum < 0:
            strength *= self.WALL_MOMENTUM_DAMP
        if abs(wall_gap) > max(2.5, self.WALL_EXTREME_GAP_MULT * float(self.spread)):
            strength *= self.WALL_EXTREME_DAMP

        if strength < self.WALL_PERSISTENCE_FLOOR * 0.75:
            return None, 0.0

        return self.wall_fair_ewma, strength

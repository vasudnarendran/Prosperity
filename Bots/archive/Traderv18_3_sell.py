...
class TomatoesTrader(BaseProductTrader):
    ...
    def take_edge(
        ...
        if side == "SELL" and predicted_edge >= self.SELL_HOLD_EDGE:
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            ...
            if sell_quote is not None and predicted_edge >= self.SELL_HOLD_EDGE and position <= target_position + 6:
            ...
            if buy_quote is not None and predicted_edge <= -self.STRONG_TREND_EDGE and position >= -8:
                buy_quote -= 1
        elif regime == "range" and sell_quote is not None and predicted_edge >= self.SELL_STRONG_HOLD_EDGE and position <= 8:
            sell_quote += 1

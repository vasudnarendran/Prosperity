...
class EmeraldsTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        for key in ("BASE_CARRY_WEIGHT", "TREND_CARRY_BONUS", "STRETCH_FADE_BONUS"):
            raw_value = os.environ.get(f"V23_{key}")
            if raw_value is not None:
                try:
                    setattr(self, key, float(raw_value))
                except ValueError:
                    pass
...
class TomatoesTrader(BaseProductTrader):
    ...
    def hybrid_weight(
        ...
        weight = self.BASE_CARRY_WEIGHT

        if carry_regime in {"trend_up", "trend_down"}:
            weight += self.TREND_CARRY_BONUS * max(0.5, fit_quality)

        if fade_regime in {"stretch", "range"} and abs(normalized_deviation) >= self.RANGE_DEVIATION_THRESHOLD:
            weight -= self.STRETCH_FADE_BONUS * min(1.0, abs(normalized_deviation) / self.STRONG_DEVIATION_THRESHOLD)

        if fade_regime == "volatile":
            weight -= self.VOLATILE_FADE_BONUS

        if carry_regime == "range" and abs(predicted_edge) < 0.75:
            weight -= 0.05

        return max(0.15, min(0.85, weight))

    def hybrid_view(self) -> Dict[str, float]:
        ...
        carry_weight = self.hybrid_weight(
            carry_regime,
            predicted_edge,
            fit_quality,
            fade_regime,
        ...
        fade_weight = 1.0 - carry_weight

        target_position = int(round((carry_weight * carry_target) + (fade_weight * fade_target)))
        adjusted_fair = (carry_weight * carry_adjusted_fair) + (fade_weight * fade_adjusted_fair)
        buy_edge = (
            carry_weight
            * self.carry_take_edge("BUY", carry_regime, predicted_edge, fit_quality, volatility)
            + fade_weight
            * self.fade_take_edge("BUY", fade_regime, fade_normalized, volatility)
        )
        sell_edge = (
            carry_weight
            * self.carry_take_edge("SELL", carry_regime, predicted_edge, fit_quality, volatility)
            + fade_weight
            * self.fade_take_edge("SELL", fade_regime, fade_normalized, volatility)
        )
        quote_edge = (
            carry_weight * self.carry_quote_edge(carry_regime, volatility, fit_quality)
            + fade_weight * self.fade_quote_edge(fade_regime, fade_normalized, volatility)
            + self.HYBRID_QUOTE_BONUS
        )
        passive_size = max(
            1,
            int(round((carry_weight * self.CARRY_PASSIVE_SIZE) + (fade_weight * self.FADE_PASSIVE_SIZE))),
        )
        max_take_size = max(
            1,
            int(round((carry_weight * self.CARRY_MAX_TAKE_SIZE) + (fade_weight * self.FADE_MAX_TAKE_SIZE))),
        )

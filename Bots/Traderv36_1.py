...
TOMATOES = {
    ...
    "TAKE_EV_VETO_TOXIC": 0.42,
    "TAKE_EV_VETO_STRETCH": 1.10,
    "TAKE_EV_VETO_BASE": 0.06,
    "TAKE_EV_VETO_FAVOR_BONUS": -0.04,
    "TAKE_EV_VETO_AGAINST_BONUS": 0.12,
    "TAKE_EV_VETO_TOXIC_BONUS": 0.08,
...
class Trader:
    ...
    def should_apply_take_ev_veto(self, regime: Dict[str, float], stretch: float) -> bool:
        return regime["toxic"] >= TOMATOES["TAKE_EV_VETO_TOXIC"] or abs(stretch) >= TOMATOES["TAKE_EV_VETO_STRETCH"]

    def take_ev_veto_margin(self, side: str, regime: Dict[str, float], stretch: float) -> float:
        margin = TOMATOES["TAKE_EV_VETO_BASE"]
        dominant = self.dominant_regime_bucket(regime)

        if dominant == "R":
            margin += 0.05
        elif dominant == "T":
            margin += TOMATOES["TAKE_EV_VETO_TOXIC_BONUS"]
        elif dominant == "U":
            margin += TOMATOES["TAKE_EV_VETO_FAVOR_BONUS"] if side == "BUY" else TOMATOES["TAKE_EV_VETO_AGAINST_BONUS"]
        elif dominant == "D":
            margin += TOMATOES["TAKE_EV_VETO_FAVOR_BONUS"] if side == "SELL" else TOMATOES["TAKE_EV_VETO_AGAINST_BONUS"]

        if stretch > TOMATOES["TAKE_EV_VETO_STRETCH"]:
            margin += 0.10 if side == "BUY" else -0.02
        elif stretch < -TOMATOES["TAKE_EV_VETO_STRETCH"]:
            margin += 0.10 if side == "SELL" else -0.02

        if regime["toxic"] >= TOMATOES["TAKE_EV_VETO_TOXIC"]:
            margin += 0.05
    ...
    def trade_tomatoes(
        ...
        apply_take_veto = self.should_apply_take_ev_veto(regime, stretch)
        ...
        buy_priority = self.aggressive_priority("BUY", book, taker_fair, buy_edge)
        sell_priority = self.aggressive_priority("SELL", book, taker_fair, sell_edge)

        first_side = "BUY" if buy_priority >= sell_priority else "SELL"
        ...
        for side in (first_side, second_side):
            ...
            if apply_take_veto:
                take_ev = self.aggressive_expected_value(
                    side,
                    book,
                    projected,
                    target_position,
                ...
                passive_ev = pre_buy_ev if side == "BUY" else pre_sell_ev
                passive_floor = max(0.0, passive_ev if passive_ev is not None else -999.0)
                if take_ev <= passive_floor + self.take_ev_veto_margin(side, regime, stretch):
                    continue
            self.sweep_book(
                side,
                book,

...
class TomatoesTrader(BaseProductTrader):
    ...
    def regime_probabilities(self) -> Tuple[Dict[str, float], int]:
        trend_score = (
            0.36 * self.clamp_prob(abs(self.momentum) / 2.5)
            + 0.26 * self.clamp_prob(abs(self.short_momentum) / 1.4)
            + 0.24 * self.clamp_prob(abs(self.imbalance) / 0.35)
            + 0.14 * self.clamp_prob(float(self.spread) / 14.0)
        )
        range_score = (
            0.42 * (1.0 - self.clamp_prob(abs(self.momentum) / 2.5))
            + 0.26 * (1.0 - self.clamp_prob(abs(self.imbalance) / 0.35))
            + 0.20 * (1.0 - self.clamp_prob(float(self.spread) / 14.0))
            + 0.12 * (1.0 - self.clamp_prob(abs(self.short_momentum) / 1.4))
        )
        high_vol_score = (
            0.45 * self.clamp_prob(float(self.spread) / 14.0)
            + 0.35 * self.clamp_prob(abs(self.momentum - self.short_momentum) / 1.8)
            + 0.20 * self.clamp_prob(abs(self.imbalance) / 0.50)
        )
        ...
        trend_signal = (0.9 * self.momentum) + (0.5 * self.short_momentum) + (1.5 * self.imbalance)
        if trend_signal > 0.2:
            trend_direction = 1
    ...
    def take_edge(self, side: str, regime: str) -> float:
        ...
        edge += 0.12 * probabilities["high_vol"]

        if side == "BUY" and trend_direction > 0:
            edge -= 0.15 * probabilities["trend"]
        if side == "SELL" and trend_direction < 0:
            edge -= 0.15 * probabilities["trend"]
    ...
    def passive_size(self, side: str, regime: str) -> int:
        ...
        size = max(1, int(round(size * (1.0 - 0.10 * probabilities["high_vol"]))))
        ...
        if side == "BUY" and trend_direction > 0 and probabilities["trend"] >= 0.45:
            size += 1
        if side == "SELL" and trend_direction < 0 and probabilities["trend"] >= 0.45:
            size += 1
    ...
    def take_orders(self, regime: str, target_position: int, adjusted_fair: float) -> Tuple[bool, bool]:
        ...
        took_sell = False
        probabilities, trend_direction = self.regime_probabilities()
        take_scale = max(0.80, 1.0 - 0.10 * probabilities["high_vol"])

        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime) and self.buy_capacity > 0:
            take_limit = max(2, int(round(self.MAX_TAKE_SIZE * take_scale)))
            if regime == "trend_up":
                take_limit += 2 if trend_direction > 0 and probabilities["trend"] >= 0.45 else 1

...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_SOFT_BAND_1_RATIO": 0.20,
    "TREND_SOFT_BAND_2_RATIO": 0.40,
    "TREND_SOFT_BAND_3_RATIO": 0.60,
    "TREND_EXIT_SLICE_SMALL": 2,
    "TREND_EXIT_SLICE_MEDIUM": 4,
    "TREND_EXIT_SLICE_LARGE": 7,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def trend_soft_bands(self) -> Tuple[int, int, int]:
        band_1 = max(1, int(self.position_limit * self.TREND_SOFT_BAND_1_RATIO))
        band_2 = max(band_1 + 1, int(self.position_limit * self.TREND_SOFT_BAND_2_RATIO))
        band_3 = max(band_2 + 1, int(self.position_limit * self.TREND_SOFT_BAND_3_RATIO))
    ...
    def trend_exit_slice(self, regime: str, side: str) -> Optional[int]:
        position = self.projected_position()
        band_1, band_2, band_3 = self.trend_soft_bands()
    ...
    def take_orders(self, regime: str, target_position: int, adjusted_fair: float) -> Tuple[bool, bool]:
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime) and self.buy_capacity > 0:
            ...
            if regime != "mean_revert" and self.projected_position() >= target_position:
                ...
                staged_limit = self.trend_exit_slice(regime, "BUY")
                if staged_limit == 0:
                    quantity = 0
                elif staged_limit is not None:
                    quantity = min(quantity, staged_limit)
                before = self.buy_capacity
                self.add_buy(int(self.best_ask), quantity)
                took_buy = self.buy_capacity < before
        ...
        if int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime) and self.sell_capacity > 0:
            ...
            if regime != "mean_revert" and self.projected_position() <= target_position:
                ...
                staged_limit = self.trend_exit_slice(regime, "SELL")
                if staged_limit == 0:
                    quantity = 0
                elif staged_limit is not None:
                    quantity = min(quantity, staged_limit)

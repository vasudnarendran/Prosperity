...
class EmeraldsTrader(BaseProductTrader):
    ...
    def fair_value(self) -> float:
        ...
        if int(self.best_ask) <= 10000:
            fair += 0.30
        if int(self.best_bid) >= 10000:
            fair -= 0.30
        return fair

    def adjusted_fair_value(self) -> float:
        adjusted = self.fair_value() - (self.position * self.INVENTORY_SKEW)

        if int(self.best_ask) <= 10000 and self.position < 0:
            adjusted += 0.40
    ...
    def passive_quotes(self, adjusted_fair: float) -> Tuple[Optional[int], Optional[int]]:
        ...
        if int(self.best_ask) <= 10000 and self.position <= 0:
            ...
            sell_quote = None
        elif int(self.best_bid) >= 10000 and self.position >= 0:
            if sell_quote is not None:
    ...
    def run(self) -> List[Order]:
        ...
        if int(self.best_ask) <= adjusted_fair - self.take_edge("BUY") and self.buy_capacity > 0:
            take_limit = self.MAX_TAKE_SIZE + (2 if int(self.best_ask) <= 10000 else 0)
        ...
        if int(self.best_bid) >= adjusted_fair + self.take_edge("SELL") and self.sell_capacity > 0:
            take_limit = self.MAX_TAKE_SIZE + (2 if int(self.best_bid) >= 10000 else 0)

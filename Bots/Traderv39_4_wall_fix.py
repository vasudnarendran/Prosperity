...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.current_wall_fair: Optional[float] = None
        self.current_wall_strength = 0.0
    ...
    def refresh_wall_state(self) -> None:
        ...
        if self.wall_strength_ewma < self.WALL_PERSISTENCE_FLOOR:
            self.current_wall_fair = None
            self.current_wall_strength = 0.0
            return

        self.current_wall_fair = self.wall_fair_ewma
        self.current_wall_strength = min(1.0, self.wall_strength_ewma)

    def blended_wall_fair(self) -> Tuple[Optional[float], float]:
        return self.current_wall_fair, self.current_wall_strength

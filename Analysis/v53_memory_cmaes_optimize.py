#!/usr/bin/env python3
"""
Focused CMA-ES for the v53 confidence-memory concept.

Why focused?
- v53 only adds a small memory layer on top of v52.
- We therefore tune only the new memory parameters and score them against the v52 baseline.
- If the optimizer cannot beat or match v52 cleanly, the concept is not worth promoting.
"""
...
BOT_SOURCE_PATH = ROOT / "Bots" / "Traderv53.py"
...
PARAM_NAMES = [
    "SIGNAL_MEMORY_DECAY",
    "FORECAST_MEMORY_DECAY",
    "CONFIDENCE_AGREE_WEIGHT",
    "TAKE_CONFIDENCE_SCALE",
    "QUOTE_CONFIDENCE_SCALE",
]

PARAM_BOUNDS: Dict[str, tuple[float, float]] = {
    "SIGNAL_MEMORY_DECAY": (0.65, 0.90),
    "FORECAST_MEMORY_DECAY": (0.72, 0.94),
    "CONFIDENCE_AGREE_WEIGHT": (0.35, 0.82),
    "TAKE_CONFIDENCE_SCALE": (0.04, 0.28),
    "QUOTE_CONFIDENCE_SCALE": (0.06, 0.34),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Focused regularized CMA-ES for Traderv53 memory parameters.")
    parser.add_argument("--max-iter", type=int, default=4, help="Number of CMA-ES generations.")
...
def run_with_params(source: str, params: Dict[str, float], day: int) -> float:
    ...
    tmp_stem = f"v53_memory_cmaes_{uuid.uuid4().hex}"
...
def save_results(
    ...
    lines = [
        "V53 Memory Focused CMA-ES Report",
        "================================",
        "",

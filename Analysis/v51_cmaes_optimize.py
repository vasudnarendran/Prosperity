#!/usr/bin/env python3
"""
Regularized CMA-ES for the reconstructed Traderv51 model.

Why regularized?
- v37 is the current official best.
- Local backtests are informative but not perfectly aligned with the official simulator.
- We therefore optimize for local average PnL while penalizing:
  - regressions on either local day
  - excessive drift away from v37 defaults

Usage:
    python3 Analysis/v51_cmaes_optimize.py
    python3 Analysis/v51_cmaes_optimize.py --max-iter 6 --population 8 --parents 4
    python3 Analysis/v51_cmaes_optimize.py --max-iter 1 --population 4 --parents 2 --output-prefix v51_cmaes_smoke
"""
...
BOT_SOURCE_PATH = ROOT / "Bots" / "Traderv51.py"
...
PARAM_BOUNDS: Dict[str, tuple[float, float]] = {
    "GAMMA_RANGE": (0.20, 1.10),
    "RESERVATION_SCALE": (0.005, 0.10),
    "RANGE_RESERVATION_BIAS": (0.05, 0.60),
    "SPREAD_VOL_COEF": (0.05, 0.60),
    "SPREAD_INV_COEF": (0.50, 1.80),
    "SPREAD_TIME_COEF": (0.80, 2.60),
    "BASE_TAKE_EDGE": (0.40, 1.40),
    "ALPHA_EDGE_SCALE": (0.90, 1.90),
    "ALPHA_IMBALANCE_SCALE": (0.30, 1.20),
    "INVENTORY_SKEW": (0.001, 0.020),
    "SOFT_LIMIT_RATIO": (0.35, 0.80),
    "PRESSURE_BIAS_SCALE": (0.05, 0.40),
    "BREAKOUT_FOLLOW_SCALE": (0.05, 0.32),
    "BREAKOUT_QUOTE_TIGHTEN": (0.05, 0.30),
    "BREAKOUT_HOLD_BONUS": (0.04, 0.20),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Regularized CMA-ES for Traderv51.")
    parser.add_argument("--max-iter", type=int, default=6, help="Number of CMA-ES generations.")
    parser.add_argument("--population", type=int, default=8, help="Population size (lambda).")
    parser.add_argument("--parents", type=int, default=4, help="Number of selected parents (mu).")
    parser.add_argument("--sigma0", type=float, default=0.10, help="Initial CMA-ES sigma.")
    parser.add_argument("--seed", type=int, default=51, help="Random seed.")
    parser.add_argument(
        "--regression-penalty",
        type=float,
        default=2.5,
        help="Penalty multiplier for local day regressions vs the reconstructed v37 baseline.",
    )
    parser.add_argument(
        "--imbalance-penalty",
        type=float,
...
def run_with_params(source: str, params: Dict[str, float], day: int) -> float:
    ...
    tmp_stem = f"v51_cmaes_{uuid.uuid4().hex}"
...
def main() -> None:
    ...
    if not math.isclose(base_day1, BASELINES["day_neg1"], abs_tol=1e-6) or not math.isclose(base_day2, BASELINES["day_neg2"], abs_tol=1e-6):
        print(
            "WARNING: source baseline differs from the expected v37 reconstruction baseline. "

...
TRADER_PATH = WORKSPACE_ROOT / "Bots" / "archive" / "Traderv27_1_local.py"
...
RUN_OUTPUT_ROOT = OUTPUT_DIR / "v27_3_combo_runs"
REPORT_PATH = OUTPUT_DIR / "v27_3_combo_report.txt"
CSV_PATH = OUTPUT_DIR / "v27_3_combo_results.csv"
...
def load_defaults() -> Dict[str, Dict[str, float]]:
    ...
    spec = importlib.util.spec_from_file_location("traderv27_3_combo_module", TRADER_PATH)
...
def build_cases(defaults: Dict[str, Dict[str, float]]) -> List[Dict[str, object]]:
    ...
    cases: List[Dict[str, object]] = [
        {"label": "baseline_v27_3", "family": "baseline", "overrides": base},
        make_case(
            base,
            "spread_plus",
            "designed",
    ...
    sampled = set()
    while len(sampled) < 72:
        combo = (
            rng.choice([1.04, 1.06, 1.08]),  # ALPHA_EDGE_SCALE
            rng.choice([1.12, 1.16, 1.20]),  # ALPHA_IMBALANCE_SCALE
            rng.choice([0.98, 1.00, 1.03, 1.06]),  # ALPHA_THRESHOLD_SCALE
            rng.choice([1.20, 1.25, 1.30]),  # BASE_TAKE_EDGE
            rng.choice([2.35, 2.40, 2.50]),  # BASE_QUOTE_EDGE
            rng.choice([0.08, 0.12, 0.16]),  # RESERVATION_SCALE
            rng.choice([0.02, 0.04, 0.08]),  # TREND_RESERVATION_BIAS
            rng.choice([0.16, 0.20, 0.24]),  # RANGE_RESERVATION_BIAS
            rng.choice([0.56, 0.64, 0.72]),  # SPREAD_VOL_COEF
            rng.choice([0.24, 0.28, 0.32]),  # SPREAD_INV_COEF
            rng.choice([0.56, 0.64, 0.72]),  # SPREAD_TIME_COEF
            rng.choice([0.20, 0.24, 0.28]),  # TREND_SELL_HOLD_EXTRA
            rng.choice([0.04, 0.08, 0.12]),  # TREND_BUY_TAKE_EXTRA
            rng.choice([1.0, 2.0]),  # TREND_QUOTE_LIFT_EXTRA
            rng.choice([0.0, 0.08, 0.12]),  # HOLD_TIME_COEF
            rng.choice([0.28, 0.34, 0.40]),  # GAMMA_RANGE
            rng.choice([0.08, 0.10, 0.12]),  # GAMMA_TREND
            rng.choice([0.32, 0.40, 0.48]),  # GAMMA_VOLATILE
        )
        sampled.add(combo)
...
def main() -> None:
    ...
    for index, case in enumerate(cases, start=1):
        result = run_backtest(
            run_id=f"v27-3-combo-{index:03d}",
    ...
    baseline = next(result for result in results if result["label"] == "baseline_v27_3")
    ...
    lines = [
        "V27.3 combination sweep",
        "",
        (
            f"Baseline v27.3 Rust PnL: {baseline['total']:.0f} | "
    ...
    lines.extend(
        [
            ...
            "- This sweep tries combination-style improvements around the current best bot instead of only isolated single-lever changes.",
            "- It focuses on interactions between spread control, imbalance scaling, take edge, reservation control, and trend-side hold behavior.",
        ]
    )
